...
create view [dbo].[__build_permit_open] as 
--create view __build_permit_open_ as 
...
inner join 
	(select * from property_val WHERE      
	(prop_val_yr IN	(SELECT appr_yr FROM  pacs_oltp.dbo.pacs_system))) AS pv on pba.prop_id=pv.prop_id
left join 
	appraiser ap 
