create view __dashboard_2021 as 
SELECT DISTINCT 
...
imprv_fix.recalc_error_validate_flag																					as recalc_error_flag,
	...
	situs.situs_street_sufix,situs.situs_unit,situs.situs_city,situs.situs_state,situs.situs_zip,situs.building_num,situs.sub_num
...
(SELECT prop_id, imprv_id,
		...
		CAST(SUM(Bonus_Room)			AS INT)		as Bonus_Room,
		...
		CAST(SUM(Comm_basement_semifinished) 					AS INT) 	as Comm_basement_semifinished
			...
			FROM 
				(SELECT row_number() over (partition by prop_id order by "imprv_id" ASC) as "num", prop_id, imprv_id, imprv_det_id, 
					...
					CASE WHEN rtrim(imprv_det_type_cd)    = 'MA'	THEN imprv_det_area ELSE 0 END AS living_area, 
					...
					CASE WHEN rtrim(imprv_det_type_cd) = 'Bonus Room'		THEN imprv_det_area ELSE 0 END AS Bonus_Room,
					...
					CASE WHEN rtrim(imprv_det_type_cd)like'C-BSMTSFIN'		THEN imprv_det_area ELSE 0 END AS Comm_basement_semifinished
						...
						FROM pacs_oltp.dbo.imprv_detail 
							...
							IN ('MA', 'BSMT', 'Bonus Room','ATTGAR', 'DETGAR', 'U-BSMT','carport','polebldg','deck'	, 'mezzFD'	, 'mezzFO'	, 'mezzLCU'	, 'mezzSF'	
...
left join 
		...
		CASE WHEN rtrim(imprv_det_type_cd)		= 'MA'	THEN 'MA' ELSE 0 END			AS living_area, 
		...
		CASE WHEN rtrim(imprv_det_type_cd)	= 'Bonus Room'		THEN 'Bonus Room'		ELSE 0 END			AS Bonus_Room,
...
LEFT JOIN 
	(SELECT vw.prop_id, ROW_NUMBER() over (partition by vw.prop_id ORDER BY vw.bldg_permit_id DESC) AS order_id,vw.bldg_permit_status, vw.bldg_permit_issue_dt, vw.bldg_permit_active, building_permit.bldg_permit_cmnt,
		building_permit.bldg_permit_desc, vw.bldg_permit_num, building_permit.bldg_permit_dt_complete, building_permit.bldg_permit_val
