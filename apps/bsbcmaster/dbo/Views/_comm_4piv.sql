create view _comm_4piv as 
SELECT DISTINCT 
pv.prop_id																							as ParcelID,  
convert(varchar(20), sales.sl_dt, 101)				as 'Sale_Date', 
sales.sl_price										as 'Sale_Price', 
sales.excise_number									as 'Excise_Number', 
sales.sl_type_cd									as 'Sale_Code', 
sales.num_days_on_market							as 'Days_on_Mkt', 
rtrim(property.geo_id)							as 'Parcel_ID', 
' '												as 'Economic_Unit',
situs.situs_display								as 'Situs_Address', 
'Benton'										as 'Situs_County',
pv.legal_acreage								as 'land_acres', 
pp.state_cd										as 'property_code',
z.zoning										as 'zoning',
imprv_details.num_units									as 'number_of_units',
imprv_fix.imprv_desc									as 'structure_type',
--pv.subset_cd as 'structure_type',
pp.living_area									as 'structure_sqft',
pp.class_cd										as 'quality',
pp.condition_cd									as 'condition',
imprv_items.Comm_frame 							as 'structure_framing_class',
...
,occ.occupancy_pct,occ.occupancy_description,occ.class,occ.occupancy_code
...
LEFT JOIN (SELECT chg_of_owner_prop_assoc.prop_id, sale.num_days_on_market,sl_price,  sale.sl_ratio_type_cd, sale.sl_county_ratio_cd, sale.sl_dt, sale.adjusted_sl_price,sale.chg_of_owner_id,sale.land_only_sale,
...
LEFT JOIN 
	(SELECT [prop_id] ,[prop_val_yr],[imprv_id], sum([unit_price]) as unit_price, SUM([num_units]) as num_units, sum(imprv_det_cost_unit_price) as imprv_det_cost_unit_price,sum(net_rentable_area) as net_rentable_area,
	...
	--[CENTROID_X] as XCoord
     -- ,[CENTROID_Y] as YCoord
		FROM [Benton_spatial_data].[dbo].[parcel]) as coords
			...
			and pv.sup_num=0
		--and pv.property_use_cd between '47' and '49'
