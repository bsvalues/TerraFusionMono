Create View __aAppraisalEst_2020_DORSales as
SELECT DISTINCT 
...
left join
	...
	(SELECT [Parcel_ID],ROW_NUMBER() over (partition by prop_id ORDER BY [OBJECTID] DESC) AS order_id,[Prop_ID],[Shape].STCentroid().STX as XCoord,	[Shape].STCentroid().STY as YCoord 
		...
		--and pv.sub_type = 'lh'
		and sales.sl_dt>='05/01/2019'
		and sales.sl_dt<='05/01/2020'
		--and sales.sl_ratio_type_cd='00'
		--and pv.prop_id=44476
		--and ad.addr_city like'Richland'
		--and pv.cycle=1
