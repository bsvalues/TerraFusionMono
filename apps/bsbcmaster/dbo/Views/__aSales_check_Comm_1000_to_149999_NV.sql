--create view __aSales_check_Ag_150000_to_249999		as
--create view __aSales_check_Ag_250000_to_999999999		as
--create view __aSales_check_Ag_1000_to_149999			as
--create view __aSales_check_Comm_150000_to_249999		as
--create view __aSales_check_Comm_250000_to_999999999	as
--create view __aSales_check_Comm_1000_to_149999		as
--create view __aSales_check_Res_150000_to_249999		as
--create view __aSales_check_Res_250000_to_999999999	as
--create view __aSales_check_Res_1000_to_149999			as

--create view __aSales_check_Ag_150000_to_249999_V		as
--create view __aSales_check_Ag_250000_to_999999999_V		as
--create view __aSales_check_Ag_1000_to_149999_V			as
--create view __aSales_check_Comm_150000_to_249999_V		as
--create view __aSales_check_Comm_250000_to_999999999_V	as
--create view __aSales_check_Comm_1000_to_149999_V		as
--create view __aSales_check_Res_150000_to_249999_V		as
--create view __aSales_check_Res_250000_to_999999999_V	as
--create view __aSales_check_Res_1000_to_149999_V			as



--create view __aSales_check_Comm_150000_to_249999_NV			as
--create view __aSales_check_Comm_250000_to_999999999_NV	as
create view __aSales_check_Comm_1000_to_149999_NV		as

--create view __aSales_check_Res_150000_to_249999_NV		as
--create view __aSales_check_Res_250000_to_999999999_NV		as
--create view __aSales_check_Res_1000_to_149999_NV				as
SELECT DISTINCT 
...
case when pv.market > 0 then CAST(ROUND((pv.market / sales.adjusted_sl_price), 2) as decimal(10, 2)) else 0 end	as Adjusted_Ratio,
case when pv2.market > 0 then CAST(ROUND((pv2.market / sales.adjusted_sl_price), 2) as decimal(10, 2)) else 0 end	as Prior_Adjusted_Ratio,
...
sales.sl_price,
sales.adjusted_sl_price,
pv.market as TotalValue,
pv2.market as market_Before,
convert(char(20), sales.sl_dt, 101) as saledate,
...
ta.tax_area_id	as tax_area_id,--ta.tax_area_number	as tca,
...
--Commercial imprvments
--imprv_items.COMM_Units	as Comm_Units,imprv_items.COMM_Tank_Type as Comm_Tank_Type,imprv_items.COMM_Tank_Capacity as Comm_Tank_Capacity,imprv_items.COMM_Service_Pit as Comm_Service_Pit,imprv_items.COMM_HVAC as Comm_HVAC, 
--imprv_items.COMM_Elevators	As Comm_Elevator, imprv_items.Comm__Sprinkler	as Comm_Sprinkler,imprv_items.Comm_frame	as Comm_Class_Description,imprv_items.COMM_Shape	as Comm_Shape, 
--imprv_items.Comm_Shape_units as Comm_Shape_units,imprv_items.COMM_HVAC_units as COMM_HVAC_units,imprv_details.net_rentable_area	as net_rentable_area,
...
--pv.ag_use_val as 'ag_use_val',pv2.ag_use_val as 'Previous_ag_use_val',imprv_fix.adjusted_val as 'Imprv_AdjVal',imprv_fix.flat_val as 'flat_value',pv.market	as 'TotalMarketValue'
...
--,id.add_factor	as adj_factor,ID.yr_new as year_new,
--id.depreciation_yr as effective_year
--,id.yr_new,id.use_up_for_pct_base	as use_up_for_pct_of_base_flag,id.stories_multiplier as stories_multiplier_flag,ID.imprv_det_adj_factor,
--id.load_factor ,ID.imprv_det_adj_val
-- ,ID.imprv_det_cost_unit_price 
--, ID.imprv_det_ms_unit_price,
-- iadj.imprv_adj_amt,iadj.imprv_adj_desc,
--iadj.imprv_adj_method,iadj.imprv_adj_pc,
...
--case when(pv2.imprv_hstd_val + pv2.imprv_non_hstd_val) <> 0 then cast(round(((pv.imprv_hstd_val + pv.imprv_non_hstd_val)
-- /(pv2.imprv_hstd_val + pv2.imprv_non_hstd_val)) * 100 - 100, 2) 	as decimal(10, 2)) else 0 end	as 'Imprv_pct_chg',
--case when(pv2.land_hstd_val + pv2.land_non_hstd_val) <> 0 then cast(round(((pv.land_hstd_val + pv.land_non_hstd_val) / (pv2.land_hstd_val + pv2.land_non_hstd_val)) * 100 - 100, 2) 
--	as decimal(10, 2)) else 0 endas 'land_pct_chg',
case when pv.market >  0 and pp.living_area>0  and sales.sl_dt>='01/01/2018' and sales.sl_ratio_type_cd='00'then CAST(ROUND((sales.sl_price / pp.living_area), 2) as decimal(10, 2)) 
else 0 end	as 'Sale_PricePerSqFt',
...
case when pv.market > 0 and pp.living_area>0 then CAST(ROUND(((pv.imprv_hstd_val + pv.imprv_non_hstd_val) / pp.living_area), 2) as decimal(10, 2)) else 0 end as 'Impv_PricePerSqft',

case when pv.market > 0 and pp.living_area>0 then CAST(ROUND(((pv.land_hstd_val + pv.land_non_hstd_val) / pv.market), 2) as decimal(10, 2)) else 0 end as 'Land_percent',
case when pv.market > 0 and pp.living_area>0 then CAST(ROUND(((pv.imprv_hstd_val + pv.imprv_non_hstd_val) / pv.market), 2) as decimal(10, 2)) else 0 end as 'Imprv_percent',
...
--and pv.hood_cd like '5%'
	and pv.hood_cd like '6%'
		...
		--and pv.sub_type = 'lh'
		and sales.sl_dt between '05/01/2018' and '05/01/2019'
	and sales.sl_price between '1000' and '149999'
		--and sales.sl_price between '150000' and '249999'
		--and sales.sl_price between '250000' and '999999999'
		--and sales.sl_ratio_type_cd='00'
	and sales.sl_ratio_type_cd not like '00'
