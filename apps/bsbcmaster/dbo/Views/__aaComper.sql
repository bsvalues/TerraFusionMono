...
Create view __aaComper as 
	SELECT DISTINCT 
pv.prop_id																							as ParcelID,  
rtrim(property.geo_id)																				as MapNumber, 
rtrim(replace(situs.situs_display, char(13) + char(10), ''))															as situs_display, 
...
rtrim(imprv_items.FixtureCount)                                                                     as fixtures,
...
sum_imprv_areas.Pole_building																		as Pole_building,
	imprv_items.COMM_Units																								as Units,
	imprv_items.COMM_Tank_Type																							as Tank_Type,
	imprv_items.COMM_Tank_Capacity																						as Tank_Capacity,
	imprv_items.COMM_Service_Pit																						as Service_Pit,
	imprv_items.COMM_HVAC																								as COMM_HVAC, 
	imprv_items.COMM_Elevators																							As Elevator, 
	imprv_items.Comm__Sprinkler																							as Sprinkler,
...
case when pv.market > 0 then CAST(ROUND((pv.market / sales.sl_price), 2) 
...
ap1.appraiser_full_name																				as 'last_appraiser',
	CAST(ISNULL(pv.imprv_hstd_val + pv.imprv_non_hstd_val, 0) - ISNULL(pv2.imprv_hstd_val + pv2.imprv_non_hstd_val, 0)  AS numeric(10, 2)) / 		
	(CASE WHEN  pv2.imprv_hstd_val + pv2.imprv_non_hstd_val IS NULL THEN 1 WHEN pv2.imprv_hstd_val + pv2.imprv_non_hstd_val = 0  
		THEN 1 ELSE pv2.imprv_hstd_val + pv2.imprv_non_hstd_val END) * 100/100						as 'imprv_percent_change',
	CAST(ISNULL(pv.land_hstd_val + pv.land_non_hstd_val, 0) - ISNULL(pv2.land_hstd_val + pv2.land_non_hstd_val, 0) 		AS numeric(10, 2))/ 
	(CASE WHEN  pv2.land_hstd_val + pv2.land_non_hstd_val IS NULL THEN 1 WHEN pv2.land_hstd_val + pv2.land_non_hstd_val = 0  
		THEN 1 ELSE pv2.land_hstd_val + pv2.land_non_hstd_val END) * 100/100  						as'land_percent_change',
	(pv.market - pv2.market	)																		as 'gain/loss',
...
left  join 
	appraiser ap1
		on pv.last_appraiser_id = ap1.appraiser_id
LEFT JOIN 
	property_val pv2
