create view __a_Sales_2015_2016 as 
...
and sales.sl_dt>='01/01/2015' and sales.sl_ratio_type_cd='00'then CAST(ROUND((sales.sl_price / pp.living_area), 2) as decimal(10, 2)) else 0 end										as 'Sale_PricePerSqFt',
...
LEFT JOIN 
	( SELECT *, ROW_NUMBER() OVER (PARTITION BY prop_id ORDER BY imprv_val DESC) AS row_id 
		FROM [pacs_oltp].[dbo].imprv 
			WHERE [pacs_oltp].[dbo].imprv.prop_val_yr =
			
												2016 --(select appr_yr from [pacs_oltp].[dbo].pacs_system) 
			--and sale_id=0 
			)as imprv_fix 
				ON land.prop_id = imprv_fix.prop_id AND imprv_fix.row_id = 1 AND imprv_fix.prop_val_yr = 
				
												2016--(select appr_yr from [pacs_oltp].[dbo].pacs_system)
LEFT JOIN 
	[pacs_oltp].[dbo].property_profile pp
		ON land.prop_id = pp.prop_id AND pp.prop_val_yr = 
		
												2016--(select appr_yr from [pacs_oltp].[dbo].pacs_system) 
LEFT JOIN
	[pacs_oltp].[dbo].property_profile pp2
		ON land.prop_id = pp2.prop_id AND pp2.prop_val_yr =	
									
												2015 --(select tax_yr from [pacs_oltp].[dbo].pacs_system) 
LEFT JOIN 
	[pacs_oltp].[dbo].property_val pv
		ON land.prop_id = pv.prop_id AND pv.prop_val_yr = 
...
LEFT JOIN 
	property_val pv2
		ON land.prop_id = pv2.prop_id AND pv2.prop_val_yr = 
				...
				CASE WHEN rtrim(imprv_det_type_cd) = 'polebldg' THEN imprv_det_area ELSE 0 END AS Pole_building
					FROM [pacs_oltp].[dbo].imprv_detail 
						WHERE[prop_val_yr] = 
														...
														2017--(select appr_yr from [pacs_oltp].[dbo].pacs_system)
...
LEFT JOIN 
	[pacs_oltp].[dbo].land_detail 
		ON property.prop_id = land_detail.prop_id AND land_detail.prop_val_yr = 
...
--and pv.hood_cd like '5%'
	--and pv.hood_cd like '6%'
		...
		--and pv.sub_type = 'lh'
		and sales.sl_dt between'01/01/2015'and '01/01/2016'
