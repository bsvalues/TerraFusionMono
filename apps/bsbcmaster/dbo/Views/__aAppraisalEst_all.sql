...
create view [dbo].[__aAppraisalEst_all] as 
SELECT DISTINCT 
...
case when pv.market > 0 and pp.living_area>0  and sales.sl_dt>='01/01/2016' and sales.sl_ratio_type_cd='00'
	then CAST(ROUND(((pv.land_hstd_val + pv.land_non_hstd_val) / pv.market), 2) as decimal(10, 2)) else 0 end				as 'Land_percent',
case when pv.market > 0 and pp.living_area>0  and sales.sl_dt>='01/01/2016' and sales.sl_ratio_type_cd='00'
	then CAST(ROUND(((pv.imprv_hstd_val + pv.imprv_non_hstd_val) / pv.market), 2) as decimal(10, 2)) else 0 end				as 'Imprv_percent',
...
LEFT JOIN
		...
		CAST(SUM(Pole_building) AS INT) 		as SWE
...
left join 
		...
		CASE WHEN rtrim(imprv_det_type_cd) = 'SWE' THEN 'polebldg' ELSE 0 END AS SWE
