create view New_Const_Current as
SELECT DISTINCT pv.prop_id as nc_prop_id, a.file_as_name, ta.tax_area_number,	pv.prop_val_yr,
(wpov.new_val_hs + wpov.new_val_nhs + wpov.new_val_p) as New_Value,
--ncp.bldg_permit_id,ncp.bldg_permit_val,ncp.property_use_cd, ncp.appraiser_full_name, ncp.hood_cd, ncp.cycle
	ncp.*
...
inner join New_construction_Permits ncp on
	ncp.prop_id=pv.prop_id

WHERE pv.prop_val_yr = (select appr_yr  from [pacs_oltp].[dbo].pacs_system) AND 
