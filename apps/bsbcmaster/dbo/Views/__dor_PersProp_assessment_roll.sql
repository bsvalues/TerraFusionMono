--2.18 DOR Assessment Roll for Personal Property 
 ...
 create view __dor_PersProp_assessment_roll as
...
LEFT OUTER JOIN 
	property_sub_type pst  ON pv.sub_type = pst.property_sub_cd 
		WHERE pv.prop_val_yr = (select appr_yr from pacs_oltp.dbo.pacs_system) AND (  pv.prop_inactive_dt IS NULL  OR udi_parent = 'T'  )  AND p.prop_type_cd = 'p'  AND isnull(pst.state_assessed_utility, 0) <> 1  AND isnull(pst.local_assessed_utility, 0) <> 1 
			--ORDER BY p.geo_id 
