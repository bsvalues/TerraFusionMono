...
CREATE view VerifyAssessedVal_vw
as
select 
	pv.prop_id  ,
	...
	0 as owner_id,
	0 as entity_id,
	case 	when assessed_val < 0 then 'AV_LT_0'
		when assessed_val > market then 'AV_GT_MV'
		when assessed_val <> market-((ag_market + timber_market)- (ag_use_val + timber_use))-ten_percent_cap
			then 'AV_SUM'
		end as check_cd,
	0 as ic_ref_id
...
where (
	assessed_val < 0 
	or assessed_val > market
	or assessed_val <> market-((ag_market + timber_market)- (ag_use_val + timber_use))-ten_percent_cap
) 
