create view __a_Sales_2016_2017 as 

SELECT DISTINCT 
pv.prop_id	as ParcelID,  
rtrim(property.geo_id)as MapNumber, pv.prop_id	as prop_id
, situs.situs_display	as situs_display, pv.hood_cd as neighborhood
, pv.subset_cd as subset,rtrim(replace(pv.cycle, char(13) + char(10), ''))as Reval,
ta.tax_area_description	as tax_area,ta.tax_area_id	as tax_area_id,--ta.tax_area_number	as tca,
rtrim(replace(abs_subdv.abs_subdv_desc, char(13) + char(10), ''))as abs_subdv_desc
,rtrim(pp.property_use_cd)as property_use_cd,rtrim(property.prop_type_cd)as prop_type_cd
,RTRIM(property_use_desc)as property_use_desc,rtrim(imprv_fix.imprv_type_cd)as PrimaryImprovement,imprv_fix.imprv_state_cd as imprv_state_cd, 

--property id's
--ava	as 'ava',fema.zone as 'flood_zone',sd.districtna as 'school_district',z.zoning	as 'zoning',
--Improvments
--imprv_fix.stories as 'stories', imprv_fix.num_imprv as 'num_imprv', pp.living_area as 'TotalArea', ID.sketch_area,ID.calc_area,sum_imprv_areas.finished_basement as 'finished_basement',sum_imprv_areas.unfinished_basement	as 'unfinished_basement',
--CAST(ISNULL(sum_imprv_areas.finished_basement, 0) + ISNULL(sum_imprv_areas.unfinished_basement, 0) AS numeric(18, 0))as Total_Basement,rtrim(imprv_fix.actual_year_built)as YearBuilt, pp.actual_age as Age,
--rtrim(pp.class_cd)as 'class_cd',rtrim(pp.class_cd) + ' ' + rtrim(pp.imprv_det_sub_class_cd)as 'class_subclass_cd',rtrim(pp.condition_cd)as 'Condition',[imprv_det_desc]as 'imprv_det_desc',
--imprv attributes
--rtrim(REPLACE(replace(imprv_fix.imprv_desc, char(10), ''), char(13), ''))as 'Style', rtrim(imprv_items.RoofCovering)as 'Roofing', rtrim(imprv_items.HVAC)as 'Heating', rtrim(imprv_items.ExtWall)as 'ExtWall',imprv_items.Bathrooms as 'Bathrooms', imprv_items.HalfBaths as 'HalfBaths', imprv_items.Fireplace as 'fireplace', imprv_items.Foundation	as 'foundation', 
--rtrim(imprv_items.Fixture_Count)as 'fixture_count',rtrim(imprv_items.FixtureCount)as 'fixture_Cost',imprv_items.Fireplace_Cost as 'fireplace_cost', imprv_items.Solar_Panels as 'Solar_Panels',sum_imprv_areas.attached_garage	as 'attached_garage', sum_imprv_areas.detached_garage	as 'detached_garage',sum_imprv_areas.carport as 'carport',sum_imprv_areas.Pole_building as 'Pole_building',
...
,sales.recorded_dt
,sales.comment
,sales.continue_current_use
,YEAR(sl_dt) as sales_year,
sl_adj_rsn,
sales.wac_cd,sales.exemption_amount
,sales.sl_ratio_cd_reason,deed_type_cd
,consideration,deed_num,deed_book_page
,sales.pers_prop_val,sales.include_reason
,sales.frozen_characteristics
,sales.sl_price
,pv.market	as 'TotalMarketValue'
,pv2.market as 'PreviousMarket',
--,pv.imprv_hstd_val + pv.imprv_non_hstd_val	as 'ImpVal',pv2.imprv_hstd_val + pv2.imprv_non_hstd_val	AS 'ImpVal_before', --imprv_fix.imprv_val_source	as 'ImpVal_source', 	 
--rtrim(ID.depreciated_replacement_cost_new)as Depr_RCN,rtrim(ID.imprv_det_calc_val)as RCN, 
--,sales.sl_class_cd as Class_cd_at_sale,sales.sl_imprv_unit_price as Imprv_unit_price_at_sale,rtrim(sales.sl_land_unit_price)	as Land_unit_price_at_sale

--Cost info like price per sq ft etc...
...
--case when pv.market > 0 and pp.living_area>0 then CAST(ROUND((pv.market / pp.living_area), 2) as decimal(10, 2)) else 0 end	as PPSF,
--case when pv.market > 0 and pp.living_area>0 then CAST(ROUND(((pv.imprv_hstd_val + pv.imprv_non_hstd_val) / pp.living_area), 2) as decimal(10, 2)) else 0 end	as BPPSF,
--Values
--rtrim(imprv_details.imprv_det_cost_unit_price)as 'Current_unit_price',imprv_details.imprv_det_cost_unit_price- sales.sl_imprv_unit_price as 'unit_price_diff',
--pv.ag_use_val as 'ag_use_val',pv2.ag_use_val as 'Previous_ag_use_val',imprv_fix.adjusted_val as 'Imprv_AdjVal',imprv_fix.flat_val as 'flat_value',pv.market	as 'TotalMarketValue',pv2.market as 'PreviousMarket'
--,pv.imprv_hstd_val + pv.imprv_non_hstd_val	as 'ImpVal',pv2.imprv_hstd_val + pv2.imprv_non_hstd_val	AS 'ImpVal_before', --imprv_fix.imprv_val_source	as 'ImpVal_source', 	 
--rtrim(ID.depreciated_replacement_cost_new)as Depr_RCN,rtrim(ID.imprv_det_calc_val)as RCN, 

--rtrim(ID.physical_pct)as adj_physical_pct,rtrim(ID.economic_pct)as adj_economic_pct,rtrim(ID.functional_pct)as adj_functional_pct,id.add_factor	as adj_factor,ID.yr_new as year_new,
--id.depreciation_yr as effective_year,id.yr_new,id.use_up_for_pct_base	as use_up_for_pct_of_base_flag,id.stories_multiplier as stories_multiplier_flag,ID.imprv_det_adj_factor,
--id.load_factor ,ID.imprv_det_adj_val ,ID.imprv_det_cost_unit_price , ID.imprv_det_ms_unit_price, iadj.imprv_adj_amt,iadj.imprv_adj_desc,iadj.imprv_adj_method,iadj.imprv_adj_pc,
--iadj.imprv_adj_seq,iadj.imprv_adj_type_cd,
--land
rtrim(pp.land_unit_price)as 'Land_UnitVal',pv.land_hstd_val + pv.land_non_hstd_val as 'LandVal',pv2.land_hstd_val + pv2.land_non_hstd_val	
AS 'LandVal_before', pp.ls_table	as 'LandVal_source',land.state_cd	as 'Land_State_cd',pv.legal_acreage as 'TotalAcres',pp.land_sqft as 'land_sqft',
--new value, new construction and permits
imprv_fix.imp_new_val AS new_val, id.new_value	as imprv_detail_new_value,ID.new_value_flag as new_value_flag,pp.percent_complete 
as percent_complete,rtrim(permits.bldg_permit_status)as permit_status, permits.bldg_permit_dt_complete	as permit_complete_date,
--bldg_permit_active	as active_permits,permits.bldg_permit_issue_dt as permit_issue_date,rtrim(REPLACE(replace(permits.bldg_permit_num, char(10), ''), char(13),''))as permit_num, 
--rtrim(REPLACE(replace(permits.bldg_permit_desc, char(10), ''), char(13),''))as permit_desc, rtrim(REPLACE(replace(permits.bldg_permit_cmnt, char(10), ''), char(13),''))as permit_cmnt, 
--changes (values)
--(pv.market - pv2.market)as 'gain_loss',case when pv2.market <> 0 then cast(round((pv.market / pv2.market) * 100 - 100, 2) as decimal(10, 2)) else 0 end	as 'pct_chg'
--,(pv.imprv_hstd_val + pv.imprv_non_hstd_val)-(pv2.imprv_hstd_val + pv2.imprv_non_hstd_val)	as 'imprv_gain_loss', (pv.land_hstd_val + pv.land_non_hstd_val)-(pv2.land_hstd_val + pv2.land_non_hstd_val)as 'land_gain_loss',
--case when(pv2.imprv_hstd_val + pv2.imprv_non_hstd_val) <> 0 then cast(round(((pv.imprv_hstd_val + pv.imprv_non_hstd_val) /(pv2.imprv_hstd_val + pv2.imprv_non_hstd_val)) * 100 - 100, 2) 	as decimal(10, 2)) else 0 end									as 'Imprv_pct_chg',
--case when(pv2.land_hstd_val + pv2.land_non_hstd_val) <> 0 then cast(round(((pv.land_hstd_val + pv.land_non_hstd_val) / (pv2.land_hstd_val + pv2.land_non_hstd_val)) * 100 - 100, 2) 	as decimal(10, 2)) else 0 end										as 'land_pct_chg',
case when pv.market >  0 and pp.living_area>0  
and sales.sl_dt>='01/01/2016' and sales.sl_ratio_type_cd='00'then CAST(ROUND((sales.sl_price / pp.living_area), 2) as decimal(10, 2)) else 0 end										as 'Sale_PricePerSqFt',
case when pv.market > 0 and pp.living_area>0 then CAST(ROUND((pv.market / pp.living_area), 2) as decimal(10, 2)) else 0 end	as 'PricePerSqFt',
case when pv.market > 0 and pp.living_area>0 then CAST(ROUND(((pv.imprv_hstd_val + pv.imprv_non_hstd_val) / pp.living_area), 2) as decimal(10, 2)) else 0 end 
as 'Impv_PricePerSqft',
--errors and mass adjustments
--imprv_mass_adj_factor	as mass_adj_factor,imprv_fix.recalc_error_validate_flag	as recalc_error_flag,	
--situs	
--situs.primary_situs,	situs.situs_num,	situs.situs_street_prefx,	situs.situs_street,	situs.situs_street_sufix,situs.situs_unit,situs.situs_city,situs.situs_state
--,situs.situs_zip,situs.building_num,situs.sub_num,
--images and spatial coords
...
LEFT JOIN (SELECT chg_of_owner_prop_assoc.prop_id, sale.sl_price,  sale.sl_ratio_type_cd, sale.sl_county_ratio_cd, sale.sl_dt, sale.adjusted_sl_price,
sale.chg_of_owner_id,sale.land_only_sale,sale.sl_qualifier,sale.sl_land_unit_price,sale.sl_class_cd,sale.sl_imprv_unit_price
,sale.sl_type_cd,sale.pers_prop_val,sale.sl_adj_rsn,sale.sl_land_type_cd,sale.continue_current_use,
			chg_of_owner.excise_number,chg_of_owner.recorded_dt,chg_of_owner.comment,sale.wac_cd,
			chg_of_owner.deed_type_cd,sale.exemption_amount,sale.sl_ratio_cd_reason,
			chg_of_owner.consideration,chg_of_owner.deed_num,chg_of_owner.deed_book_page,sale.include_reason,sale.frozen_characteristics,
...
LEFT JOIN 
	( SELECT *, ROW_NUMBER() OVER (PARTITION BY prop_id ORDER BY imprv_val DESC) AS row_id 
		FROM [pacs_oltp].[dbo].imprv 
			WHERE [pacs_oltp].[dbo].imprv.prop_val_yr =2017 --(select appr_yr from [pacs_oltp].[dbo].pacs_system) 
			--and sale_id=0 
			) 
			as imprv_fix 
				ON land.prop_id = imprv_fix.prop_id AND imprv_fix.row_id = 1 AND imprv_fix.prop_val_yr = 2017--(select appr_yr from [pacs_oltp].[dbo].pacs_system)
LEFT JOIN 
	[pacs_oltp].[dbo].property_profile pp
		ON land.prop_id = pp.prop_id AND pp.prop_val_yr = 2017--(select appr_yr from [pacs_oltp].[dbo].pacs_system) 
LEFT JOIN
	[pacs_oltp].[dbo].property_profile pp2
		ON land.prop_id = pp2.prop_id AND pp2.prop_val_yr =2016 --(select tax_yr from [pacs_oltp].[dbo].pacs_system) 
LEFT JOIN 
	[pacs_oltp].[dbo].property_val pv
		ON land.prop_id = pv.prop_id AND pv.prop_val_yr = 2017--(select appr_yr from [pacs_oltp].[dbo].pacs_system) 
left join 
...
LEFT JOIN 
	property_val pv2
		ON land.prop_id = pv2.prop_id AND pv2.prop_val_yr = 2016--(select tax_yr from [pacs_oltp].[dbo].pacs_system)
...
left join 
		...
		CASE WHEN rtrim(imprv_det_type_cd) = 'polebldg' THEN 'polebldg' ELSE 0 END AS Pole_building
			FROM [pacs_oltp].[dbo].imprv_detail 
				WHERE[prop_val_yr] = 2017--(select appr_yr from [pacs_oltp].[dbo].pacs_system)
		...
		max(CASE WHEN i_attr_val_id = 56 THEN i_attr_unit END)							as COMM_Elevator_unit	
			FROM [pacs_oltp].[dbo].imprv_attr
				GROUP BY prop_id, imprv_id, prop_val_yr ) as imprv_items 
					ON imprv_fix.prop_id = imprv_items.prop_id AND imprv_fix.imprv_id = imprv_items.imprv_id 
					AND imprv_items.prop_val_yr = 2017--(select appr_yr from[pacs_oltp].[dbo]. pacs_system) 
left join
	(SELECT distinct [prop_id]  ,[prop_val_yr],[imprv_id], [imprv_det_id]  ,[sup_num] ,[sale_id]  ,[imprv_det_class_cd],[imprv_det_meth_cd]  ,[imprv_det_type_cd] 
	,[seq_num] ,[imprv_det_val]	,[imprv_det_val_source],[imprv_det_desc] ,[imprv_det_area]  ,[imprv_det_area_type] ,[calc_area] ,[sketch_area] ,  imprv_det_calc_val,
	 depreciated_replacement_cost_new ,physical_pct, economic_pct,functional_pct,new_value,	new_value_flag,add_factor,depreciation_yr, yr_new,use_up_for_pct_base
	,stories_multiplier, imprv_det_adj_factor ,imprv_det_adj_val,dep_pct_override,physical_pct_override,functional_pct_override,economic_pct_override
    ...
    FROM [pacs_oltp].[dbo].[imprv_detail]
	 where prop_val_yr=2017--(select appr_yr from pacs_oltp.dbo.pacs_system)
			...
			imprv_items.prop_val_yr = 2017--(select appr_yr from [pacs_oltp].[dbo].pacs_system) 
...
LEFT JOIN 
	[pacs_oltp].[dbo].land_detail 
		ON property.prop_id = land_detail.prop_id AND land_detail.prop_val_yr = 2017--(select appr_yr from [pacs_oltp].[dbo].pacs_system)
left join
...
LEFT JOIN 
	(SELECT vw.prop_id, ROW_NUMBER() over (partition by vw.prop_id ORDER BY vw.bldg_permit_id DESC) AS order_id,vw.bldg_permit_status, vw.bldg_permit_issue_dt, 
	vw.bldg_permit_active, building_permit.bldg_permit_cmnt,
...
left join
	(SELECT  row_number() over (partition by prop_id order by id desc) as order_id, prop_id, REPLACE( REPLACE( image_path, '\\CHPACS\OLTP\pacs_oltp\Images\','')
...
--and pv.hood_cd like '5%'
	--and pv.hood_cd like '6%'
		...
		--and pv.sub_type = 'lh'
		and sales.sl_dt between'01/01/2016'and '01/01/2017'
