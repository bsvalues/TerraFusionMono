...
CREATE VIEW [daily_cash_tendered_report_vw]
AS
...
SELECT prop_id =
	...
	T.amount AS cash_tendered, 
	tc.amount as change_given, 
	t.amount + tc.amount as total_amt, 
	B.description, 
	P.void_batch_id, 
	P.payee_id, 
...
AND T.tender_type_cd = 'C' and T.description <> 'Change Returned'

cross apply (
	select min(tender_id) as min_tender_id
	from tender with(nolock)
	where tender.payment_id = t.payment_id
	and tender_type_cd = 'C' 
	and description <> 'Change Returned'
) min_tender

cross apply (
	select isnull(sum(amount), 0) total_change
	from tender with(nolock)
	where tender.payment_id = t.payment_id
	and tender_type_cd = 'C'
	and description = 'Change Returned'
...
cross apply (
	select case when t.tender_id = min_tender_id then total_change else 0 end as amount
) tc
