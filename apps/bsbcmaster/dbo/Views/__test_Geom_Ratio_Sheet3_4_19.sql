create view __test_Geom_Ratio_Sheet3_4_19 as 
SELECT
rtrim(property.geo_id)															as'Parcel#',
pv.prop_id																		as ParcelID,  
situs.situs_display																as Street,  
...
rtrim(pp.class_cd) + ' ' + rtrim(pp.imprv_det_sub_class_cd)						as Qlty,
rtrim(pp.condition_cd)															as Cond,
sales.deed_type_cd																as Deed,
sales.excise_number																as Excise,
...
sl_ratio_type_cd																as Ratio_Code, 
sl_county_ratio_cd																as County_Ratio_Code,
...
land_only_sale ,
pp.land_unit_price																as Current_Land_Price,
sl_land_unit_price																as Land_at_Sale,
--CAST(ISNULL(pv.imprv_hstd_val + pv.imprv_non_hstd_val, 0) - ISNULL(pv2.imprv_hstd_val + pv2.imprv_non_hstd_val, 0)   AS numeric(18, 4)) / (CASE WHEN  pv2.imprv_hstd_val + pv2.imprv_non_hstd_val IS NULL  THEN 1 WHEN pv2.imprv_hstd_val + pv2.imprv_non_hstd_val = 0 THEN 1 ELSE pv2.imprv_hstd_val + pv2.imprv_non_hstd_val END) * 100/100				
 --as Imprv_change,
--CAST(ISNULL(pv.land_hstd_val + pv.land_non_hstd_val, 0) - ISNULL(pv2.land_hstd_val + pv2.land_non_hstd_val, 0) AS numeric(18, 4)) / (CASE WHEN  pv2.land_hstd_val + pv2.land_non_hstd_val IS NULL THEN 1 WHEN pv2.land_hstd_val + pv2.land_non_hstd_val = 0  THEN 1 ELSE pv2.land_hstd_val + pv2.land_non_hstd_val END) * 100/100				
	--as Land_change,
sl_type_cd,
sl_land_type_cd,
sl_qualifier,
rtrim(pp.class_cd)																as Quality,
pp.actual_age																	as Age,
imprv_fix.imp_new_val															AS New_Val, 
imprv_fix.stories																as Stories, 
imprv_fix.num_imprv																as NumberOfImproments, 
imprv_fix.imprv_state_cd														as State_Code, 
...
sum_imprv_areas.attached_garage													as 'Att_Garage', 
sum_imprv_areas.detached_garage													as 'det_Garage',
...
--pv.sub_type,
--adjusted_sl_price																as 'Adjusted_Sale_Price',
--chg_of_owner_id																as 'Change_of_Owner_Id',
...
rtrim(imprv_det_desc) = 'Main Area' )	as imprv_areas

GROUP BY prop_id, imprv_id )			as sum_imprv_areas 

ON 

imprv_fix.prop_id = sum_imprv_areas.prop_id AND imprv_fix.imprv_id = sum_imprv_areas.imprv_id
