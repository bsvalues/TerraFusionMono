create view __aaSales_ratio as

SELECT DISTINCT 
pv.prop_id	as ParcelID,  
rtrim(property.geo_id)as MapNumber, pv.prop_id	as prop_id,
case when pv.market > 0 and sales.sl_price>0			then CAST(ROUND((pv.market / sales.sl_price), 2) as decimal(10, 2)) else 0 end	as Current_Ratio,
case when pv.market > 0	and sales.adjusted_sl_price>0	then CAST(ROUND((pv.market / sales.adjusted_sl_price), 2) as decimal(10, 2)) else 0 end	as Adjusted_Ratio,
case when pv2.market > 0 and sales.adjusted_sl_price>0	then CAST(ROUND((pv2.market / sales.adjusted_sl_price), 2) as decimal(10, 2)) else 0 end	as Prior_Adjusted_Ratio,
...
convert(char(20),sales.recorded_dt, 101)  as recorded_date,
sales.comment    as  chg_comment,

sales.[reet_id],
sales.[excise_number],
sales.instrument_type_cd, 
sales.pers_prop_included, 
sales.pers_prop_val as ppv, 
sales.pers_prop_description, 
sales.exemption_claimed, 
sales.wac_number_type_cd, 
sales.wac_reason, 

sales.urban_growth_cd, 
sales.exemption_amount, 
sales.agency_id, 
sales.imp_manual_entry, 
sales.imp_partial_sale, 
sales.imp_continuance_flag, 
sales.imp_historic_flag, 
sales.imp_open_space_flag, 
sales.imp_city, 
sales.imp_current_use_flag, 
sales.imp_unique_identifier, 
sales.comment as sl_comment,
