create view __aAppraiserEst_Comper as

SELECT distinct
--Parcel identification 
	pv.prop_id as ParcelID,  
	rtrim(property.geo_id) as MapNumber, pv.prop_id as prop_id, 
	situs.situs_display as situs_display, rtrim(replace(abs_subdv.abs_subdv_desc, char(13) + char(10), '')) as abs_subdv_desc, 
	pv.hood_cd as neighborhood, pv.subset_cd as subset,
	rtrim(replace(pv.cycle, char(13) + char(10), '')) as reval,
	ta.tax_area_number,
	ta.tax_area_id,
--Property codes	
	rtrim(pp.property_use_cd) as property_use_cd, rtrim(property.prop_type_cd) as prop_type_cd, RTRIM(property_use_desc) as property_use_desc,
--Improvements
	rtrim(imprv_fix.imprv_type_cd) as PrimaryImprovement, imprv_fix.imprv_state_cd as imprv_state_cd, 
	imprv_fix.stories as stories, imprv_fix.num_imprv as num_imprv, 
	pp.living_area as TotalArea, sum_imprv_areas.finished_basement as finished_basement,
	...
	rtrim(imprv_fix.actual_year_built) as YearBuilt, pp.actual_age as Age, rtrim(pp.class_cd) as class_cd,
	rtrim(pp.class_cd) + ' ' + rtrim(pp.imprv_det_sub_class_cd) as class_subclass_cd, rtrim(pp.condition_cd) as Condition,
	rtrim(REPLACE(replace(imprv_fix.imprv_desc, char(10), ''), char(13), '')) as Style, 
	rtrim(imprv_items.RoofCovering) as Roofing, rtrim(imprv_items.HVAC) as Heating, rtrim(imprv_items.ExtWall) as ExtWall,
	imprv_items.Bathrooms as Bathrooms, imprv_items.HalfBaths as HalfBaths, 
	imprv_items.Fireplace as fireplace, imprv_items.Foundation as foundation, rtrim(imprv_items.Fixture_Count) as fixture_count,
	sum_imprv_areas.attached_garage as attached_garage, sum_imprv_areas.detached_garage as detached_garage,
	sum_imprv_areas.carport	as Carport, sum_imprv_areas.Pole_building as Pole_building,
--Yard Improv
	 yv.Yard_Imprv,
...
--Values	
	...
	rtrim(pp.land_unit_price) as Land_UnitVal, pv.ag_use_val as ag_use_val,	pv.market as TotalMarketValue,
	pv.imprv_hstd_val + pv.imprv_non_hstd_val as ImpVal, pv.land_hstd_val + pv.land_non_hstd_val as LandVal,
--Land
	pv.legal_acreage as totalacres, pp.land_sqft as land_sqft,
--Sales
case when pv.market > 0 then CAST(ROUND((pv.market / sales.sl_price), 2) as decimal(10, 2)) else 0 end	as current_ratio,	
	sales.excise_number as excise_number, sales.deed_type_cd as deed_type_cd, adjusted_sl_price as adjustedSaleprice,
	sales.sl_price as OriginalSalePrice, convert(char(20), sl_dt, 101) as SaleDate,
	sales.sl_class_cd as class_cd_at_sale, sales.sl_imprv_unit_price as Imprv_unit_price_at_sale,
	rtrim(sales.sl_land_unit_price) as Land_unit_price_at_sale, sl_ratio_type_cd as sl_ratio_type_cd, 
	sl_county_ratio_cd as sl_county_ratio_cd, land_only_sale as land_only_sale,
	--Info from GIS not in PACS yet	
	fema.zone as flood_zone, sd.districtna as school_district, z.zoning as zoning,
-- Images	
	images.img_path,
--GIS Coords
...
( SELECT prop_id, SUM(size_acres) as size_acres, prop_val_yr, state_cd
				FROM pacs_oltp.dbo.land_detail 
					GROUP BY prop_id,  prop_val_yr,state_cd
) as land
LEFT JOIN 
...
(SELECT prop_id, imprv_id,
		CAST(SUM(finished_basement) AS INT) as finished_basement, CAST(SUM(unfinished_basement) AS INT) as unfinished_basement,
		CAST(SUM(attached_garage) AS INT) as attached_garage, CAST(SUM(detached_garage) AS INT) as detached_garage,
...
) as sum_imprv_areas ON imprv_fix.prop_id = sum_imprv_areas.prop_id AND imprv_fix.imprv_id = sum_imprv_areas.imprv_id
left join 
...
left join 
	[pacs_oltp].[dbo].[__Yard_imprv_Val] yv
		on pv.prop_id=yv.prop_id
left join
...
LEFT JOIN
	(SELECT  [order_id] ,[prop_id],[image_id] ,[image_type] ,[image_location] ,[image_nm],[img_path]
  FROM [pacs_oltp].[dbo].[__Main_Image]where order_id=1 
  ) as images ON land.prop_id = images.prop_id AND images.order_id = 1
LEFT JOIN 
...
WHERE land.prop_val_yr = (select appr_yr from [pacs_oltp].[dbo].pacs_system) AND pv.prop_inactive_dt is null AND situs.primary_situs= 'Y' AND pv.sup_num=0
