create view __ApprEst_LH_8_21_reet as
...
left join 
	((SELECT  row_number() over (partition by  pacs_image.ref_id order by 	pacs_image.image_id desc) as order_id, pacs_image.ref_id, REPLACE( REPLACE( pacs_image.location, '\\CHPACS\OLTP\pacs_oltp\Images\',''), '\\CHPACS\OLTP\pacs_oltp\\','') AS reet_img_path
		FROM [pacs_oltp].[dbo].[pacs_image]
			WHERE image_type = 'reet' )) as reet
				ON land.prop_id =reet.ref_id AND images.order_id = 1
	...
	--[CENTROID_X] as XCoord
     -- ,[CENTROID_Y] as YCoord
		FROM [Benton_spatial_data].[dbo].[parcel]) as coords
			...
			and pv.sup_num=0
	and pv.property_use_cd between '80' and '99'
