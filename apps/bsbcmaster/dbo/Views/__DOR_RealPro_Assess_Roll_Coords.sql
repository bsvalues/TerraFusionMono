create view __DOR_RealPro_Assess_Roll_Coords as 
SELECT DISTINCT pv.prop_id, p.geo_id, pv.cycle, pv.hood_cd,
...
INNER JOIN
	...
	where pv1.prop_val_yr =(select appr_yr  from [pacs_oltp].[dbo].pacs_system)----change year as needed
	and pv1.sup_num <= 0----change supp num as needed
...
WHERE pv.prop_val_yr = (select appr_yr  from [pacs_oltp].[dbo].pacs_system)----change year as needed
AND pv.sup_num <= 0----change supp num as needed
AND pv.prop_inactive_dt is null 
--AND pv.cycle = 1----change cycle as needed
