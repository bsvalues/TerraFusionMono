...
create view [dbo].[__Dashboard_View] as


--create view __sp_Dashboard_View as 
SELECT DISTINCT 
...
--as neighborhood
, 
rtrim(replace(pv.cycle, char(13) + char(10), ''))													as Cycle,
...
case when pv.market <> 0 then CAST(ROUND((pv.market / sales.sl_price), 2) 	as decimal(10, 2)) else 0 end as 'Ratio',
...
sl_ratio_type_cd																					as ratio_type_cd, 
sl_county_ratio_cd																					as county_ratio_cd,
land_only_sale																						as land_only_sale,
imprv_fix.imp_new_val																				AS new_val, 
imprv_details.imprv_det_cost_unit_price																as imprv_unit_price,
...
--AS numeric(18, 4))/ (CASE WHEN  pv2.land_hstd_val + pv2.land_non_hstd_val IS NULL THEN 1 WHEN pv2.land_hstd_val + pv2.land_non_hstd_val = 0  
--THEN 1 ELSE pv2.land_hstd_val + pv2.land_non_hstd_val END) * 100/100							    as land_percent_change,
--pv2.--pv2.market																					as PreviousMarket,	
...
LEFT JOIN 
(SELECT chg_of_owner_prop_assoc.prop_id, sale.sl_price,  sale.sl_ratio_type_cd, sale.sl_county_ratio_cd, sale.sl_dt, sale.adjusted_sl_price,sale.chg_of_owner_id,sale.land_only_sale,sale.sl_qualifier,sale.sl_land_unit_price,
sale.sl_type_cd,sale.pers_prop_val,sale.sl_adj_rsn,sale.sl_land_type_cd,chg_of_owner.excise_number,chg_of_owner.deed_type_cd,ROW_NUMBER() over (partition by chg_of_owner_prop_assoc.prop_id ORDER BY sl_dt DESC) AS order_id
FROM 
...
--and sl_ratio_type_cd=00
and deed_type_cd not like 'Hdeed'and deed_type_cd not like 'spwd'and deed_type_cd not like 'trd'and deed_type_cd not like 'oth'and deed_type_cd not like 'Hdeed'and deed_type_cd not like 'qcd'and deed_type_cd not like 'odeed'
and deed_type_cd not like 'mt' ) as sales ON land.prop_id = sales.prop_id AND sl_ratio_type_cd=00 and sales.order_id = 1 LEFT OUTER JOIN split_assoc sa WITH (NOLOCK) ON land.prop_id = sa.prop_id LEFT OUTER JOIN split_into si WITH (NOLOCK) 
ON sa.split_id = si.split_id 

LEFT OUTER JOIN 

merge_assoc ma 

WITH (NOLOCK) ON land.prop_id = ma.prop_id LEFT OUTER JOIN merge_from mf WITH (NOLOCK) ON ma.merge_id = mf.merge_id INNER JOIN pacs_system ON land.prop_val_yr = pacs_system.appr_yr 

LEFT JOIN 

property 

ON land.prop_id = property.prop_id LEFT JOIN ( SELECT *, ROW_NUMBER() OVER (PARTITION BY prop_id ORDER BY imprv_val DESC) AS row_id 
FROM 

imprv 

WHERE imprv.prop_val_yr = (select appr_yr from pacs_system) ) as imprv_fix ON land.prop_id = imprv_fix.prop_id AND imprv_fix.row_id = 1 AND imprv_fix.prop_val_yr = (select appr_yr from pacs_system)
...
on pv.last_appraiser_id = ap.appraiser_id left  join appraiser ap1 on  pv.last_appraiser_id = ap1.appraiser_id

LEFT JOIN 

property_val pv2

ON land.prop_id = pv2.prop_id AND pv2.prop_val_yr = (select appr_yr-1 from pacs_system)

LEFT JOIN 

(SELECT [prop_id] ,[prop_val_yr],[imprv_id], sum([unit_price]) as unit_price, sum(imprv_det_cost_unit_price) as imprv_det_cost_unit_price,sum(net_rentable_area) as net_rentable_area,sum(new_value) as new_value
FROM [pacs_oltp].[dbo].[imprv_detail] 
GROUP BY prop_id, imprv_id,[prop_val_yr] ) as imprv_details ON imprv_fix.prop_id = imprv_details.prop_id and imprv_fix.prop_val_yr = imprv_details.prop_val_yr and imprv_fix.imprv_id = imprv_details.imprv_id
LEFT JOIN
(SELECT prop_id, imprv_id,CAST(SUM(finished_basement) AS INT) as finished_basement, CAST(SUM(unfinished_basement) AS INT) as unfinished_basement,CAST(SUM(attached_garage) AS INT) as attached_garage, CAST(SUM(detached_garage) AS INT) as detached_garage,
CAST(SUM(carport) AS INT) as carport,CAST(SUM(Pole_building) AS INT) as Pole_building

FROM 

(SELECT row_number() over (partition by prop_id order by "imprv_id" ASC) as "num", prop_id, imprv_id, imprv_det_id, 
CASE WHEN rtrim(imprv_det_desc) = 'Main Area' THEN imprv_det_area ELSE null END AS living_area, imprv_detail.imprv_det_type_cd,CASE WHEN rtrim(imprv_det_type_cd) = 'BSMT' THEN imprv_det_area ELSE null END AS finished_basement, 
CASE WHEN rtrim(imprv_det_type_cd) = 'U-BSMT' THEN imprv_det_area ELSE null END AS unfinished_basement,CASE WHEN rtrim(imprv_det_type_cd) = 'ATTGAR' THEN imprv_det_area ELSE null END AS attached_garage, 
CASE WHEN rtrim(imprv_det_type_cd) = 'DETGAR' THEN imprv_det_area ELSE null END AS detached_garage,CASE WHEN rtrim(imprv_det_type_cd) = 'carport' THEN imprv_det_area ELSE null END AS Carport,
...
WHERE[prop_val_yr] = (select appr_yr from pacs_system)AND rtrim(imprv_det_type_cd) IN ('MA', 'BSMT', 'ATTGAR', 'DETGAR', 'U-BSMT','carport','polebldg' ) OR rtrim(imprv_det_desc) = 'Main Area' ) as imprv_areas
GROUP BY prop_id, imprv_id )  as sum_imprv_areas ON imprv_fix.prop_id = sum_imprv_areas.prop_id AND imprv_fix.imprv_id = sum_imprv_areas.imprv_id LEFT JOIN 
...
ON pv.abs_subdv_cd = abs_subdv.abs_subdv_cd LEFT Join 

situs 

on pv.prop_id=situs.prop_id left join

owner 

o

on    pv.prop_id = o.prop_id and pv.prop_val_yr = o.owner_tax_yr  and pv.sup_num = o.sup_num inner join

account a

on o.owner_id=a.acct_id LEFT JOIN (SELECT vw.prop_id, ROW_NUMBER() over (partition by vw.prop_id ORDER BY vw.bldg_permit_id DESC) AS order_id,
vw.bldg_permit_status, vw.bldg_permit_issue_dt, vw.bldg_permit_active, building_permit.bldg_permit_cmnt,building_permit.bldg_permit_desc, vw.bldg_permit_num, building_permit.bldg_permit_dt_complete FROM BUILDING_PERMIT_VW as vw

LEFT JOIN 

building_permit 

ON vw.bldg_permit_id = building_permit.bldg_permit_id WHERE prop_id IS NOT NULL ) as permits 
...
ON ta.tax_area_id = wta.tax_area_id left join(SELECT  row_number() over (partition by prop_id order by id desc) as order_id, prop_id, REPLACE( REPLACE( image_path, '\\CHPACS\OLTP\pacs_oltp\Images\',''), '\\CHPACS\OLTP\pacs_oltp\\','') AS img_path
...
(SELECT 
[Parcel_ID],
ROW_NUMBER() over (partition by prop_id ORDER BY [OBJECTID] DESC) AS order_id,[Prop_ID],--Geometry,
[Shape].STCentroid().STX as XCoord,
	[Shape].STCentroid().STY as YCoord,   [CENTROID_X] as x,[CENTROID_Y] as y

FROM 
--[Benton_spatial_data].[dbo].[spatial_coords]
[Benton_spatial_data].[dbo].[parcel]
) as coords ON land.prop_id = coords.Prop_ID AND coords.order_id = 1 WHERE land.prop_val_yr = (select appr_yr  from pacs_system)  and pv.prop_inactive_dt is null
