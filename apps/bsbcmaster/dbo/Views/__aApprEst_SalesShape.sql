create view __aApprEst_SalesShape as
...
SELECT --DISTINCT 
	...
	case when pv.market > 0 then 
	CAST(ROUND((pv.market / sales.sl_price), 2) as decimal(10, 2)) else 0 end							as Current_Ratio,
...
imprv_fix.recalc_error_validate_flag																					as recalc_error_flag,
	...
	situs.primary_situs,	situs.situs_num,	situs.situs_street_prefx,	situs.situs_street,	situs.situs_street_sufix,situs.situs_unit,situs.situs_city,situs.situs_state,situs.situs_zip,situs.building_num,situs.sub_num,
		lsc.sum_of_land_segs,  		lsc.legal_acreage,		lsc.sum_of_land_seg_mrkt_val,		lsc.total_ag_unit_price	,lsc.[CONVERSION BALANCING SEGMENT] ,lsc.[Land Used by Farm Buildings   ] ,lsc.[Rangeland                     ]      ,lsc.[Nonbuildable land             ]      ,lsc.[Open Space Market Value       ]
      ,lsc.[Secondary Comm/Indust Land    ]      ,lsc.[Irrigated Pasture             ] ,lsc.[Primary Commercial/Indust Land]      ,lsc.[Homesite                      ]      ,lsc.[Irrigated Agland              ]
      ,lsc.[Utility Easement              ]  ,lsc.[Utility Towers                ]  ,lsc.[Land in transition            ]   ,lsc.[Rural Undeveloped             ]    ,lsc.[Dry Pasture                   ]
      ,lsc.[Easement                      ]  ,lsc.[Dry Agland                    ]   ,lsc.[Common Areas                  ]	,images.img_path,
	  	coords.XCoord,
	coords.YCoord, coords.shape
...
LEFT JOIN 
	...
	and pv.hood_cd not like '5%'
and pv.hood_cd not like '6%'
