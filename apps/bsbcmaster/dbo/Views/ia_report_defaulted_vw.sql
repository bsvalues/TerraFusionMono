...
CREATE  VIEW dbo.ia_report_defaulted_vw
AS
SELECT DISTINCT TOP 100 PERCENT 
	...
	dbo.account.file_as_name AS contact_name,
        (SELECT MIN(ia_dt_due) FROM installment_agreement_schedule ias WHERE ias.ia_id = ia.ia_id) AS start_date,
        (SELECT MAX(ia_dt_due) FROM installment_agreement_schedule ias WHERE ias.ia_id = ia.ia_id) AS end_date,
        (SELECT MAX(ia_dt_pd) FROM installment_agreement_schedule ias WHERE ias.ia_id = ia.ia_id) AS last_payment_date,
	ISNULL(ia.ia_payment_terms, '') AS terms,
	ISNULL(ia.ia_payment_amt, 0) AS payment_amount, 
	ISNULL((SELECT 	COUNT(ias.ia_dt_due) 
		FROM 	installment_agreement_schedule ias 
		WHERE 	ias.ia_id = ia.ia_id AND ias.ia_dt_due < GETDATE() AND (ias.ia_dt_pd IS NULL OR
                        (ISNULL(ias.ia_amt_pd, 0) < ISNULL(ias.ia_amt_due, 0)))), 0) 
	       AS num_missed, 
	ISNULL((SELECT  SUM(ias.ia_amt_due)
                FROM    installment_agreement_schedule ias
                WHERE   ias.ia_id = ia.ia_id), 0) 
	       AS total_due, 
	ISNULL((SELECT  SUM(ias.ia_amt_pd)
                FROM    installment_agreement_schedule ias
