...
CREATE view [dbo].[__aAppraiserEst_Residential_sales_2023] as
...
SELECT DISTINCT 
	...
	case when pv.market > 0 and sales.sl_dt>='01/01/2020' and sales.sl_ratio_type_cd='00'
	then CAST(ROUND((pv.market / sales.sl_price), 2) as decimal(10, 2)) else 0 end											as Current_Ratio,
	case when pv.market >  0 and pp.living_area>0  and sales.sl_dt>='01/01/2020' and sales.sl_ratio_type_cd='00'
  ...
  ,ID.dep_pct_override
  ,ID.physical_pct_override
  ,ID.functional_pct_override
  ,ID.economic_pct_override
  ,ID.size_adj_pct,size_adj_pct_override
  ,ID.imprv_det_cost_unit_price
  , ID.imprv_det_ms_unit_price, iadj.imprv_adj_amt,iadj.imprv_adj_desc,iadj.imprv_adj_method,iadj.imprv_adj_pc,iadj.imprv_adj_seq,iadj.imprv_adj_type_cd,
...
CAST(ISNULL(pv2.market, 0) - ISNULL(pv.market, 0)  AS numeric(18, 4)) / (CASE WHEN pv.market IS NULL THEN 1 WHEN pv.market = 0 THEN 1 ELSE pv.market END) * 100/-100 AS 'percent_change',
...
case when pv.market >  0 and pp.living_area>0  and sales.sl_dt>='01/01/2020' and sales.sl_ratio_type_cd='00'
	then CAST(ROUND((sales.sl_price / pp.living_area), 2) as decimal(10, 2)) else 0 end										as 'Sale_PricePerSqFt',
case when pv.market > 0 and pp.living_area>0 
	then CAST(ROUND((pv.market / pp.living_area), 2) as decimal(10, 2)) else 0 end											as 'PricePerSqFt',
case when pv.market > 0 and pp.living_area>0 
	then CAST(ROUND(((pv.imprv_hstd_val + pv.imprv_non_hstd_val) / pp.living_area), 2) as decimal(10, 2)) else 0 end		as 'Impv_PricePerSqft',
...
LEFT JOIN
	[pacs_oltp].[dbo].property_profile pp2
		ON land.prop_id = pp2.prop_id AND pp2.prop_val_yr = (select appr_yr-1 from [pacs_oltp].[dbo].pacs_system) 
LEFT JOIN 
