...
pp.eff_yr_blt as effective_yr,
pp.actual_year_built as pp_yr_built,
rtrim(pp.class_cd) as class_cd,
rtrim(pp.imprv_det_sub_class_cd) as sub_class_cd,  
...
(SELECT  row_number() over (partition by prop_id order by id desc) as order_id, prop_id, REPLACE( REPLACE( image_path, '\\CHPACS\OLTP\pacs_oltp\Images\',''), '\\CHPACS\OLTP\pacs_oltp\\','') AS reet_img_path

FROM 

[web_internet_benton].[dbo].[_clientdb_property_image]WHERE image_type = 'reet' ) as reet_doc ON pv.prop_id = reet_doc.prop_id 
