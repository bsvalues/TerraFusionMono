...
CREATE VIEW dbo.supp_entity_avg_imprv_land_hs_vw
AS
SELECT entity_prop_assoc.entity_id, entity_prop_assoc.sup_num, 
    entity_prop_assoc.tax_yr, CONVERT(numeric(14, 0), 
    SUM(property_val.imprv_hstd_val + property_val.land_hstd_val) 
...
FROM entity_prop_assoc INNER JOIN
    ...
    property_val.imprv_hstd_val IS NOT NULL AND
    property_val.land_hstd_val IS NOT NULL INNER JOIN
