...
(SELECT  
    [prop_id],
      [error]
      ,[error_id]
  FROM [pacs_oltp].[dbo].[prop_recalc_errors]
where sup_yr=(select appr_yr from pacs_system)--Change if you want soecific years
  )     as basedata
  pivot (
  max(error_id)
  for error
  in ([Matrix UC missing or zero.  Stopping calculation of this detail.]
...
,[Properties whose market value is > $500 cannot have a U500 exemption.])) as pivottable
