Create view __aSimple_Comper as
		...
		CAST(SUM(pool) 					AS INT) 	as pool
			...
			FROM 
				(SELECT row_number() over (partition by prop_id order by "imprv_id" ASC) as "num", prop_id, imprv_id, imprv_det_id, 
					...
					CASE WHEN rtrim(imprv_det_type_cd) = 'pool'			THEN imprv_det_area ELSE 0 END AS pool

						FROM pacs_oltp.dbo.imprv_detail 
							WHERE[prop_val_yr] = (select appr_yr from [pacs_oltp].[dbo].pacs_system)AND rtrim(imprv_det_type_cd) 
							IN ('MA', 'BSMT', 'ATTGAR', 'DETGAR', 'U-BSMT','carport','polebldg','swe','pool'	) 
...
left join 
		...
		CASE WHEN rtrim(imprv_det_type_cd)	= 'pool'		THEN 'pool'			ELSE 0 END			AS pool

			FROM [pacs_oltp].[dbo].imprv_detail 
				WHERE[prop_val_yr] = (select appr_yr from [pacs_oltp].[dbo].pacs_system)AND rtrim(imprv_det_type_cd) 
					IN ('MA', 'BSMT', 'ATTGAR', 'DETGAR', 'U-BSMT','carport','polebldg','swe','pool') OR rtrim(imprv_det_desc) = 'Main Area'  
