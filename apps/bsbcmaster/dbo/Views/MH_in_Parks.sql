create view  MH_in_Parks as 
SELECT distinct
pv.prop_val_yr													as 'prop_val_yr',
pv.prop_id as 'ParcelID', 
pa.link_type_cd as link_type,
pa.child_prop_id as rel_prop_id,
p.geo_id as 'Geo ID', 
pv.hood_cd as 'NBHD',
...
idt.calc_area																													as calc_area,
	...
	bldg_permit_val																						as bldg_permit_val,
...
INNER JOIN imprv_detail idt WITH (nolock) ON
	...
	AND idt.sale_id = 0
	AND idt.imprv_det_type_cd = 'MHOME'
inner join imprv i
...
LEFT OUTER JOIN situs s WITH (nolock) ON
	...
	(SELECT  child_prop_id,pv.prop_id,
      ...
      ,pa.sup_num ,pa.prop_val_yr ,lOrder ,link_type_cd,link_sub_type_cd
  ...
  pv.prop_id=pa.parent_prop_id and pv.prop_val_yr=pa.prop_val_yr
   where  pv.prop_val_yr=(select appr_yr from pacs_system)) as pa
		...
		max(CASE WHEN i_attr_val_id = 56 THEN i_attr_unit END)							as COMM_Elevator_unit	
			FROM [pacs_oltp].[dbo].imprv_attr
				GROUP BY prop_id, imprv_id, prop_val_yr ) as imprv_items 
					...
					AND imprv_items.prop_val_yr = (select appr_yr from[pacs_oltp].[dbo]. pacs_system) 
left join
	(SELECT distinct [prop_id]
      ,[prop_val_yr],[imprv_id],[imprv_det_id] ,[sup_num]  ,[sale_id] ,[imprv_det_class_cd],[imprv_det_meth_cd]  ,[imprv_det_type_cd] ,[seq_num] 
	  ,[imprv_det_val] ,[imprv_det_val_source] ,[imprv_det_desc] ,[imprv_det_area] ,[imprv_det_area_type] ,[calc_area] ,[sketch_area] ,  imprv_det_calc_val, 
	  depreciated_replacement_cost_new  ,physical_pct, economic_pct,functional_pct,new_value,new_value_flag
...
LEFT JOIN 
	(SELECT [Parcel_ID],ROW_NUMBER() over (partition by prop_id ORDER BY [OBJECTID] DESC) AS order_id,[Prop_ID],[Shape].STCentroid().STX as XCoord,	
	[Shape].STCentroid().STY as YCoord 
		FROM [Benton_spatial_data].[dbo].[PARCELSANDASSESS]) as coords
			ON pa.child_prop_id = coords.Prop_ID AND coords.order_id = 1
