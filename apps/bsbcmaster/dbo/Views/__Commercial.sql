...
Create view [dbo].[__Commercial] as
--Create view __Commercial as
...
FROM 
		(SELECT prop_id, SUM(size_acres) as size_acres, prop_val_yr, state_cd
			FROM [pacs_oltp].[dbo].land_detail where prop_val_yr=(select appr_yr from pacs_oltp.dbo.pacs_system) and sale_id=0
...
and pv.hood_cd like '6%'
	...
	--and ap.appraiser_full_name ='Jennifer Luce'
		...
		--and ap.appraiser_full_name ='Unknown'
		...
		--and ap.appraiser_full_name ='Mark Peterson'
		--and ap.appraiser_full_name ='Oliver Querin'
