create view Current_Changes as

SELECT        pv1.prop_id, pv1.cycle, pv1.prop_val_yr AS year_before
			, pv1.market AS market_val_yr_before, pv2.prop_val_yr AS year_after
			, pv2.market AS market_val_yr_after, CAST(ISNULL(pv2.market, 0) - ISNULL(pv1.market, 0) 
			 ...
			 , pp.land_acres, pp.imprv_type_cd, pv1.imprv_hstd_val + 
...
property_val AS pv1 
INNER JOIN
prop_supp_assoc AS psa1 ON psa1.prop_id = pv1.prop_id AND psa1.owner_tax_yr = pv1.prop_val_yr AND psa1.sup_num = pv1.sup_num 
INNER JOIN
