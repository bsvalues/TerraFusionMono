 ...
 create view __aApprEst_prior_year as 
SELECT DISTINCT 
...
imprv_fix.recalc_error_validate_flag																					as recalc_error_flag,
	...
	case when(pv2.land_hstd_val + pv2.land_non_hstd_val) <> 0 	then cast(round(((pv.land_hstd_val + pv.land_non_hstd_val) / 
	(pv2.land_hstd_val + pv2.land_non_hstd_val)) * 100 - 100, 2) 	as decimal(10, 2)) else 0 end										as 'land_pct_chg',
	situs.primary_situs,situs.situs_num,situs.situs_street_prefx,situs.situs_street,situs.situs_street_sufix,situs.situs_unit,situs.situs_city,
	situs.situs_state,situs.situs_zip,situs.building_num,situs.sub_num,
	images.img_path,
	coords.XCoord,
	coords.YCoord
	...
	FROM 
		(SELECT prop_id, SUM(size_acres) as size_acres, prop_val_yr, state_cd
			FROM [pacs_oltp].[dbo].land_detail 
				where prop_val_yr=(select appr_yr-1 from pacs_oltp.dbo.pacs_system) and sale_id=0
...
LEFT JOIN 
	( SELECT *, ROW_NUMBER() OVER (PARTITION BY prop_id ORDER BY imprv_val DESC) AS row_id 
		FROM [pacs_oltp].[dbo].imprv 
			WHERE [pacs_oltp].[dbo].imprv.prop_val_yr = (select appr_yr-1 from [pacs_oltp].[dbo].pacs_system) and sale_id=0 ) as imprv_fix 
				ON land.prop_id = imprv_fix.prop_id AND imprv_fix.row_id = 1 AND imprv_fix.prop_val_yr = (select appr_yr-1 from [pacs_oltp].[dbo].pacs_system)
LEFT JOIN 
	[pacs_oltp].[dbo].property_profile pp
		ON land.prop_id = pp.prop_id AND pp.prop_val_yr = (select appr_yr-1 from [pacs_oltp].[dbo].pacs_system) 
LEFT JOIN
	[pacs_oltp].[dbo].property_profile pp2 
		ON land.prop_id = pp2.prop_id AND pp2.prop_val_yr = (select tax_yr-1 from [pacs_oltp].[dbo].pacs_system) --gives previous years info
LEFT JOIN 
	[pacs_oltp].[dbo].property_val pv
		ON land.prop_id = pv.prop_id AND pv.prop_val_yr = (select appr_yr-1 from [pacs_oltp].[dbo].pacs_system) 
left join 
...
LEFT JOIN 
	property_val pv2
		ON land.prop_id = pv2.prop_id AND pv2.prop_val_yr = (select tax_yr-1 from [pacs_oltp].[dbo].pacs_system)--gives previous years info
			...
			(SELECT row_number() over (partition by prop_id order by "imprv_id" ASC) as "num", prop_id, imprv_id, imprv_det_id, 
				...
				CASE WHEN rtrim(imprv_det_type_cd) = 'polebldg' THEN imprv_det_area ELSE 0 END AS Pole_building
					FROM [pacs_oltp].[dbo].imprv_detail 
						...
						AND rtrim(imprv_det_type_cd) IN ('MA', 'BSMT', 'ATTGAR', 'DETGAR', 'U-BSMT','carport','polebldg' ) OR rtrim(imprv_det_desc) = 'Main Area' ) as imprv_areas 
...
left join 
		...
		CASE WHEN rtrim(imprv_det_type_cd) = 'polebldg' THEN 'polebldg' ELSE 0 END AS Pole_building
			FROM [pacs_oltp].[dbo].imprv_detail 
				WHERE[prop_val_yr] = (select appr_yr-1 from [pacs_oltp].[dbo].pacs_system)
				AND rtrim(imprv_det_type_cd)IN ('MA', 'BSMT', 'ATTGAR', 'DETGAR', 'U-BSMT','carport','polebldg' ) OR rtrim(imprv_det_desc) = 'Main Area'  
		...
		max(CASE WHEN i_attr_val_id = 56 THEN i_attr_unit END)							as COMM_Elevator_unit	
			FROM [pacs_oltp].[dbo].imprv_attr
				GROUP BY prop_id, imprv_id, prop_val_yr ) as imprv_items 
					ON imprv_fix.prop_id = imprv_items.prop_id AND imprv_fix.imprv_id = imprv_items.imprv_id AND 
						imprv_items.prop_val_yr = (select appr_yr-1 from[pacs_oltp].[dbo]. pacs_system) 
left join
  ...
  FROM [pacs_oltp].[dbo].[imprv_detail]
	 where prop_val_yr=(select appr_yr-1 from pacs_oltp.dbo.pacs_system)
			and sale_id=0 and imprv_det_desc='Main Area') as id on imprv_fix.prop_id = id.prop_id AND imprv_fix.imprv_id = id.imprv_id AND 
				imprv_items.prop_val_yr = (select appr_yr-1 from [pacs_oltp].[dbo].pacs_system) 
LEFT JOIN 
	[pacs_oltp].[dbo].land_detail 
		ON property.prop_id = land_detail.prop_id 
			AND land_detail.prop_val_yr = (select appr_yr-1 from [pacs_oltp].[dbo].pacs_system)
left join
     ...
     -- ,[CENTROID_Y] as YCoord
		FROM [Benton_spatial_data].[dbo].[parcel]) as coords
			ON land.prop_id = coords.Prop_ID AND coords.order_id = 1
				WHERE land.prop_val_yr = (select appr_yr-1  from [pacs_oltp].[dbo].pacs_system)  
