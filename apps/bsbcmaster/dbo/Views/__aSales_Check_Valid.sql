create view __aSales_Check_Valid as
SELECT DISTINCT 
...
,land_only_sale as land_only_sale,sales.recorded_dt,sales.comment,sales.continue_current_use
...
--land
rtrim(pp.land_unit_price)as 'Land_UnitVal',pv.land_hstd_val + pv.land_non_hstd_val as 'LandVal',pv2.land_hstd_val + pv2.land_non_hstd_val	AS 'LandVal_before', pp.ls_table	as 'LandVal_source',land.state_cd	as 'Land_State_cd',pv.legal_acreage as 'TotalAcres',pp.land_sqft as 'land_sqft',
--new value, new construction and permits
imprv_fix.imp_new_val AS new_val, id.new_value	as imprv_detail_new_value,ID.new_value_flag as new_value_flag,pp.percent_complete	as percent_complete,rtrim(permits.bldg_permit_status)as permit_status, permits.bldg_permit_dt_complete	as permit_complete_date,
...
--changes (values)
--(pv.market - pv2.market	)as 'gain_loss',case when pv2.market <> 0 then cast(round((pv.market / pv2.market) * 100 - 100, 2) 	as decimal(10, 2)) else 0 end	as 'pct_chg',(pv.imprv_hstd_val + pv.imprv_non_hstd_val)-(pv2.imprv_hstd_val + pv2.imprv_non_hstd_val)	as 'imprv_gain_loss', (pv.land_hstd_val + pv.land_non_hstd_val)-(pv2.land_hstd_val + pv2.land_non_hstd_val)as 'land_gain_loss',
...
case when pv.market >  0 and pp.living_area>0  and sales.sl_dt>='01/01/2016' and sales.sl_ratio_type_cd='00'then CAST(ROUND((sales.sl_price / pp.living_area), 2) as decimal(10, 2)) else 0 end										as 'Sale_PricePerSqFt',
