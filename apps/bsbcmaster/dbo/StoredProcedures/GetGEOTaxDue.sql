...
CREATE  PROCEDURE GetGEOTaxDue 
@input_geo_id		     	varchar(100),
...
if (@input_year > 0)
begin
	DECLARE GEO_BILL CURSOR  FAST_FORWARD
	FOR select bill.bill_id
		...
		and property.geo_id = @input_geo_id
...
OPEN GEO_BILL
FETCH NEXT FROM  GEO_BILL into @bill_id
...
select geo_id = @input_geo_id, current_tax_due = @property_tax_due, 
