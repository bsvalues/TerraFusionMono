...
select @data_type_id = data_type_id from dbo.ccChecksum_TrackedData where data_desc = 'property_sketch'
print @data_type_id
exec dbo.usp_ccCheckSum_property_sketch_Compare 2,2011,-10,@data_type_id,1
*/

 
CREATE PROCEDURE dbo.usp_ccCheckSum_property_sketch_Compare
...
declare @TableName sysname 
    set @TableName = 'property_sketch'
declare @ccTemplateTable sysname
    set @ccTemplateTable = 'cc_checksum_property_sketch_template' -- this is also hard coded in one spot  
declare @new_table_name sysname
    set @new_table_name = '#ccCheckSum_property_sketch_new' -- this is also hard coded in one spot
declare @old_table_name sysname
    set @old_table_name = 'ccCheckSum_property_sketch_old'
...
select * into #ccCheckSum_property_sketch_new 
  from dbo.cc_checksum_property_sketch_template
