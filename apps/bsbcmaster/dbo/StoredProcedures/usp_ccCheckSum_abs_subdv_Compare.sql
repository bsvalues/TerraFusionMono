...
/*
exec dbo.usp_ccCheckSum_abs_subdv_Compare 2,2013,1,1,1
*/
 
CREATE PROCEDURE dbo.usp_ccCheckSum_abs_subdv_Compare
...
declare @TableName sysname 
    set @TableName = 'abs_subdv'
declare @ccTemplateTable sysname
    set @ccTemplateTable = 'cc_checksum_abs_subdv_template' -- this is also hard coded in one spot  
declare @new_table_name sysname
    set @new_table_name = '#ccCheckSum_abs_subdv_new' -- this is also hard coded in one spot
declare @old_table_name sysname
    set @old_table_name = 'ccCheckSum_abs_subdv_old'
declare @join_table_for_prop_id sysname
    set @join_table_for_prop_id = 'property_val' 
declare @join_syntax varchar(300)
    set @join_syntax = ' ON ' + @TableName + '.abs_subdv_yr = '
                     + @join_table_for_prop_id + '.prop_val_yr '   
                     + ' AND ' + @TableName + '.abs_subdv_cd = ' 
                     + @join_table_for_prop_id + '.abs_subdv_cd '
...
select * into #ccCheckSum_abs_subdv_new 
  from dbo.cc_checksum_abs_subdv_template
...
set @sql = ' insert into ' + @new_table_name + @CRLF
         ...
         + ' and ' +  @TableName + '.abs_subdv_yr =  ' + convert(varchar(4),@yr) + @CRLF + @CRLF
