...
create procedure FreezeCeilingAcceptSupplementGroupValidation
	@run_id int
as
...
-- Is the qualify year equal to the previous year (prev_yr_tax_yr)?
update
	freeze_ceiling_accept_supplement_group_run_detail
set
	action_indicator = 1,
	action_message = 'Qualify Year for the Exemption is not equal to the Previous Tax Year.',
	report_sort_order = 625,
	calculated_freeze_ceiling = null,
	calculated_freeze_yr = null
from
	freeze_ceiling_run as fcr with (nolock)
...
and	detail.action_indicator < 0
and	detail.report_sort_order < 0
and	detail.qualify_yr <> detail.prev_yr_tax_yr
where
	fcr.run_id = @run_id
