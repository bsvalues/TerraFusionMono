...
CREATE PROCEDURE ExportPropertyAccess

@input_database_name	varchar(50) = '',
@input_num_years		int = -1,
@input_min_bill_years	int = 2
...
if @input_min_bill_years < 2
	set @input_min_bill_years = 2

--*****************************************************************************
-- clean temp tables

if object_id('tempdb..#commercial_dor') is not null
	drop table #commercial_dor
if object_id('tempdb..#farm_dor') is not null
	drop table #farm_dor
if object_id('tempdb..#imprv_adj') is not null
	drop table #imprv_adj
if object_id('tempdb..#land_detail') is not null
	drop table #land_detail
if object_id('tempdb..#layer_assoc') is not null
	drop table #layer_assoc
if object_id('tempdb..#residential_dor') is not null
	drop table #residential_dor
if object_id('tempdb..#tables') is not null
	drop table #tables
if object_id('tempdb..#tmpPropVal') is not null
	drop table #tmpPropVal
if object_id('tempdb..#tmpDetail') is not null
	drop table #tmpDetail
if object_id('tempdb..#tmpImprvDetail') is not null
	drop table #tmpImprvDetail
if object_id('tempdb..#tmpImpDetVal') is not null
	drop table #tmpImpDetVal
if object_id('tempdb..#tmpMainArea') is not null
	drop table #tmpMainArea
if object_id('tempdb..#tmpDups') is not null
	drop table #tmpDups
...
print 'Exporting!'
	...
	declare @include_adjustments bit
	select
		@include_adjustments = case szConfigValue when 'T' then 1 else 0 end
	from pacs_config with (nolock)
	where szGroup = 'Property Access' and szConfigname = 'Include Prior Year ADJ'
	set @include_adjustments = isnull(@include_adjustments, 0)
...
-- fields needed in insert statement in temp tables to speed things up
	...
	outer apply (
		select top 1 i_attr_val_cd
		from imprv_attr iai with(nolock)
		where ii.imprv_id = iai.imprv_id
			and ii.prop_id = iai.prop_id
			and ii.imprv_det_id = iai.imprv_det_id
			and ii.prop_val_yr = iai.prop_val_yr
			and ii.sup_num = iai.sup_num
			and iai.i_attr_val_id = 3
	) ia
	...
	select ima.prop_val_yr, ima.sup_num, ima.sale_id, ima.prop_id, ima.imprv_id, max(ima.imprv_det_val) as imprv_det_val
	...
	on ia.prop_id = ima.prop_id
	and ia.prop_val_yr = ima.prop_val_yr
	and ia.sup_num = ima.sup_num
	and ia.sale_id = ima.sale_id
	and ia.imprv_id = ima.imprv_id
	and ia.imprv_det_id = ima.imprv_det_id	
	group by ima.prop_val_yr, ima.sup_num, ima.sale_id, ima.prop_id, ima.imprv_id	
	...
	set @sql = '
		...
		(prop_id, prop_val_yr, geo_id, prop_type_cd, prop_type_desc, dba_name, legal_desc, 
		 ...
		 property_use_cd, secondary_use_cd
		 )
	...
	if @region = @washington 
	begin
		if @include_adjustments = 1  begin -- 'new' way
			set @sql = '
				...
				where year <= ' + cast((@sys_tax_year - @input_min_bill_years) as varchar) + '
				and ((current_amount_due <> amount_paid)
				...
				and ex.year = f.year
				where f.year <= ' + cast((@sys_tax_year - @input_min_bill_years) as varchar) + ' 
				and current_amount_due <> amount_paid
				...
				and (b.year > ' + cast((@sys_tax_year - @input_min_bill_years) as varchar) + ' or isNull(ex.prop_id, 0) > 0)
			'
		end
		else -- 'old' way
			set @sql = '
	...
	begin
		...
		if @include_adjustments = 1  begin -- 'new' way
			set @sql = '
				...
				where isNull(fee.is_active, 0) = 1
				and (fee.year > ' + cast((@sys_tax_year - @input_min_bill_years) as varchar) + ' or isNull(ex.prop_id, 0) > 0)
				...
				-- DO 93976 - Without the [refund] table, calculating an adjustment credit bill causes an exception,
				-- but Nicole wanted to preserve the bugged behavior until we can evaluate further.
				--select distinct r.*
				--into ' + @input_database_name + '.dbo.[refund]
				--from [refund] as r with (nolock)
				--join ' + @input_database_name + '.dbo.[refund_transaction_assoc] as rta with (nolock)
				...
				select distinct wts.*
				into ' + @input_database_name + '.dbo.[wa_tax_statement]
				from wa_tax_statement as wts with (nolock)
				left join exportYears ex with (nolock)
				on ex.year = wts.year
				and ex.prop_id = wts.prop_id
				and isNull(wts.statement_id, -1) > -1
				where (wts.year > ' + cast((@sys_tax_year - @input_min_bill_years) as varchar) + ' or isNull(ex.prop_id, 0) > 0)
	...
	-- Start: Create all tables, views and procedures
		...
		begin
			...
			insert into #tables(tableName, populateData)
				values ('user_role', 1)
			insert into #tables(tableName, populateData)
				values ('penalty_interest_property_type', 1)
			insert into #tables(tableName, populateData)
				values ('penalty_interest_property_type_dor_use_code', 1)
			insert into #tables(tableName, populateData)
				values ('penalty_interest_property_type_imprv_det_type_cd', 1)
	...
	begin
		...
		-- BuildSQLBill (modified PA version)
		set @status = 'BuildSQLBill (PA)'
		set @sql = @input_database_name + '.dbo.sp_executesql N''
		IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N''''[dbo].[BuildSQLBill]'''') AND type in (N''''P'''', N''''PC''''))
		BEGIN
			DROP PROCEDURE BuildSQLBill
		END
		'''
		exec(@sql)
		...
		-- Copy a modified version of the [penalty_and_interest] memory table trigger, with replication allowed
		set @status = 'tr_penalty_and_interest_delete_insert_update_MemTable (PA)'

		set @sql = @input_database_name + '.dbo.sp_executesql N''
		if exists (select 1 from sys.triggers where name = ''''tr_penalty_and_interest_delete_insert_update_MemTable'''') 
		begin
			drop trigger tr_penalty_and_interest_delete_insert_update_MemTable
		end
		'''
		exec(@sql)
