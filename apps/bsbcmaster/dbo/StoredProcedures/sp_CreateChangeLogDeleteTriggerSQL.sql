...
create procedure sp_CreateChangeLogDeleteTriggerSQL
	@szTableName sysname
as

set nocount on

	declare @szTriggerName sysname
	
	set @szTriggerName = 'tr_' + @szTableName + '_delete_ChangeLog'
...
'
	...
	/* Check to see if changes are not being logged for this table */
	print 'if not exists ('
	...
	print 'declare @tvar_szOldValue varchar(255)'
	print 'set @tvar_szOldValue = ''DELETED'''
	print ''

	print 'declare @tvar_key_prop_id int'
	print ''

	declare @szFetchColumns varchar(8000)
	
	declare curColumns cursor
	for
		select sc.name, st.name, sc.length, sc.prec, sc.scale
		from sysindexkeys as sik
		join syscolumns as sc on
			sik.id = sc.id and
			sik.colid = sc.colid
	...
	begin
		...
		begin
			...
			set @szFetchColumns = @szFetchColumns + ', '
		end
		...
		begin
			set @szFetchColumns = @szFetchColumns + 'case ' + @szColName + ' when 0 then @tvar_lFutureYear else ' + @szColName + ' end'
		end
		else
		begin
			set @szFetchColumns = @szFetchColumns + @szColName
		end
	...
	print 'for'
	print '     select ' + @szFetchColumns + ' from deleted'
	print 'for read only'
	print ''
	print 'open curRows'
	...
	begin
		...
		print '     set @tvar_szOldValue = cast(@agent_id as varchar)'
		select @iColumnID = iColumnID from pacs_columns where szColumnName = 'agent_id'
	end
	else if ( @szTableName = 'arb_protest' )
	begin
		print '     select @tvar_szRefID = ''Case ID: '' + convert(varchar(12), @case_id) + ''-'' + convert(varchar(4), prop_val_yr)'
		print '     from arb_protest with(nolock)'
		print '     where case_id = @case_id and'
		print '     appr_year = @appr_year'
	end
	...
	begin
		print '     select @tvar_szRefID = entity_cd'
	...
	begin
		print '     select @tvar_szRefID = imprv_type_cd'
		print '     from deleted'
		print '     where prop_id = @prop_id and'
		print '     prop_val_yr = @prop_val_yr and'
		print '     imprv_id = @imprv_id and'
		print '     sup_num = @sup_num and'
		print '     sale_id = @sale_id'
	end
	else if ( @szTableName = 'imprv_adj' )
	begin
		print '     select @tvar_szRefID = i.imprv_type_cd + ''-'' + ia.imprv_adj_type_cd'
		print '     from imprv_adj as ia with(nolock)'
		print '     join imprv as i with(nolock) on'
		print '          i.prop_id = @prop_id'
		print '          and i.prop_val_yr = @prop_val_yr'
		print '          and i.sup_num = @sup_num'
		print '          and i.imprv_id = @imprv_id'
		print '          and i.sale_id = @sale_id'
		print '     where'
		print '          ia.prop_id = @prop_id'
		print '          and ia.prop_val_yr = @prop_val_yr'
		print '          and ia.sup_num = @sup_num'
		print '          and ia.imprv_id = @imprv_id'
		print '          and ia.sale_id = @sale_id'
		print '          and ia.imprv_adj_seq = @imprv_adj_seq'
	...
	begin
		print '     select @tvar_szRefID = id.imprv_det_class_cd + ''-'' + id.imprv_det_meth_cd + ''-'' + id.imprv_det_type_cd + ''-'' + ida.imprv_adj_type_cd'
		print '     from imprv_det_adj as ida with(nolock)'
		print '     join imprv_detail as id with(nolock) on'
		print '          id.prop_id = @prop_id'
		print '          and id.prop_val_yr = @prop_val_yr'
		print '          and id.sup_num = @sup_num'
		print '          and id.imprv_id = @imprv_id'
		print '          and id.imprv_det_id = @imprv_det_id'
		print '          and id.sale_id = @sale_id'
		print '     where'
		print '          ida.prop_id = @prop_id'
		print '          and ida.prop_val_yr = @prop_val_yr'
		print '          and ida.sup_num = @sup_num'
		print '          and ida.imprv_id = @imprv_id'
		print '          and ida.imprv_det_id = @imprv_det_id'
		print '          and ida.sale_id = @sale_id'
		print '          and ida.imprv_det_adj_seq = @imprv_det_adj_seq'
	end
	...
	begin
		print '     select @tvar_szRefID = ld.land_type_cd + ''-'' + la.land_seg_adj_type'
		print '     from land_adj as la with(nolock)'
		print '     join land_detail as ld with(nolock) on'
		print '          ld.prop_id = @prop_id'
		...
		print '     where'
		print '          la.prop_id = @prop_id'
		print '          and la.prop_val_yr = @prop_val_yr'
		print '          and la.sup_num = @sup_num'
		print '          and la.land_seg_id = @land_seg_id'
		print '          and la.sale_id = @sale_id'
		print '          and la.land_seg_adj_seq = @land_seg_adj_seq'
	end
	...
	begin
		print '     select @tvar_szRefID = ld.land_type_cd'
	...
	begin
		print '     select @tvar_szRefID = pp_type_cd'
		print '     from pers_prop_seg with(nolock)'
		print '     where prop_id = @prop_id and'
		print '     prop_val_yr = @prop_val_yr and'
		print '     sup_num = @sup_num and'
		print '     pp_seg_id = @pp_seg_id'
	end
	...
	begin
		print '     select @tvar_szRefID = convert(varchar(4), @year) + ''-'' + pps.value_method + ''-'' + pps.table_code + ''-'' + pps.segment_type + '':'' + ppsa.pp_sched_adj_cd'
		print '     from pp_schedule_adj as ppsa with(nolock)'
		print '     join pp_schedule as pps with(nolock) on'
		print '          pps.pp_sched_id = @pp_sched_id'
		print '          and pps.year = @year'
		print '     where'
		print '          ppsa.pp_sched_id = @pp_sched_id and'
		print '          ppsa.pp_sched_adj_id = @pp_sched_adj_id and'
		print '          ppsa.year = @year'
	end
	...
	begin
		print '     select @tvar_szRefID = convert(varchar(4), @year) + ''-'' + pps.value_method + ''-'' + pps.table_code + ''-'' + pps.segment_type + '':'' + ppsqd.quality_cd + ''-'' + ppsqd.density_cd'
		print '     from pp_schedule_quality_density as ppsqd with(nolock)'
		print '     join pp_schedule as pps with(nolock) on'
		print '          pps.pp_sched_id = @pp_sched_id'
		print '          and pps.year = @year'
		print '     where'
		print '          ppsqd.pp_sched_id = @pp_sched_id and'
		print '          ppsqd.pp_sched_qd_id = @pp_sched_qd_id and'
		print '          ppsqd.year = @year'
	end
	...
	begin
		print '     select @tvar_szRefID = ''Property Freeze - Year: '' + convert(varchar(4), @exmpt_tax_yr) + ''; Entity: '' + ltrim(rtrim(e.entity_cd)) + ''; Exemption: '' + ltrim(rtrim(@exmpt_type_cd))  + ''; Owner: '' + a.file_as_name'
	...
	begin
		print '     set @tvar_szRefID = ''Form Type: '' + (select form_type from deleted where lKey = @lKey)'
	end
	...
	begin
		...
		print '		set @tvar_szOldValue = cast(@fin_account_id as varchar)'
		select @iColumnID = iColumnID from pacs_columns where szColumnName = 'fin_account_id'
	end
	...
	begin
		print '     set @tvar_szRefID = ''MHM '' + cast(@mhm_id as varchar) + '' - Property '' + cast(@tvar_key_prop_id as varchar)'
	end
	else
	begin
	...
	begin
		...
		print '     values ( @tvar_lPacsUserID, system_user, host_name(), @tvar_dtNow, ''D'', ' + convert(varchar(12), @iTableID) + ', 0, @tvar_szOldValue, ''DELETED'', @tvar_szRefID )'
		print '     set @tvar_lChangeID = @@identity'
	end
	else
	begin
		print '     insert change_log with(rowlock) ( lPacsUserID, szSQLAccount, szMachineName, dtChange, szChangeType, iTableID, iColumnID, szOldValue, szNewValue, szRefID )'
		print '     values ( @tvar_lPacsUserID, system_user, host_name(), @tvar_dtNow, ''D'', ' + convert(varchar(12), @iTableID) + ', ' + convert(varchar(12), @iColumnID) +', @tvar_szOldValue, ''DELETED'', @tvar_szRefID )'
		print '     set @tvar_lChangeID = @@identity'
	end
