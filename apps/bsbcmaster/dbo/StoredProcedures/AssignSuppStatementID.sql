...
CREATE PROCEDURE AssignSuppStatementID
...
update next_statement_id  set  next_statement_id =  @statement_id + 1
			   where statement_yr = @input_sup_yr
...
begin
	...
	FOR select bill.bill_id, bill.prop_id, bill.owner_id, bill.sup_num from bill, account, property
    	where   bill.owner_id   = account.acct_id
	and     bill.prop_id    = property.prop_id
	and     bill.sup_tax_yr = @input_sup_yr
	and    ( bill.sup_num    = @input_sup_num or ( bill.sup_num   <> @input_sup_num and adjustment_code = 'BPP'))
...
begin
	DECLARE BILL SCROLL CURSOR
	FOR select bill.bill_id, bill.prop_id, bill.owner_id, bill.sup_num from bill, account, property, address
    	where  ( bill.sup_num    = @input_sup_num or ( bill.sup_num   <> @input_sup_num and adjustment_code = 'BPP'))
	and    bill.sup_tax_yr = @input_sup_yr 
	and    entity_id in (select entity_id from entity_tax_statement_group_assoc, entity_tax_statement_group
			where entity_tax_statement_group_assoc.group_id = @input_statement_group_id
			and   entity_tax_statement_group_assoc.group_id = entity_tax_statement_group.group_id
...
begin
	...
	begin
		...
		if exists (select * from bill  where prop_id     = @prop_id 
				             and    sup_num   = @input_sup_num
				             and    sup_tax_yr = @input_sup_yr
				             and    entity_id in (select entity_id from entity_tax_statement_group_assoc, entity_tax_statement_group
						              where entity_tax_statement_group_assoc.group_id = @input_statement_group_id
							and   entity_tax_statement_group_assoc.group_id = entity_tax_statement_group.group_id
							and   entity_tax_statement_group.group_yr       = @input_sup_yr)
					and    stmnt_id <> 0)
		begin
			select @statement_id = stmnt_id
			from bill  where prop_id = @prop_id 
				             and    sup_num = @input_sup_num
				             and    sup_tax_yr = @input_sup_yr
				             and    entity_id in (select entity_id from entity_tax_statement_group_assoc, entity_tax_statement_group
						              where entity_tax_statement_group_assoc.group_id = @input_statement_group_id
