...
select @data_type_id = data_type_id from dbo.ccChecksum_TrackedData where data_desc = 'land_detail_characteristic'
print @data_type_id
exec dbo.usp_ccCheckSum_land_detail_characteristic_Compare 2,2011,-10,@data_type_id,1
*/

 
CREATE PROCEDURE dbo.usp_ccCheckSum_land_detail_characteristic_Compare
...
declare @TableName sysname 
    set @TableName = 'land_detail_characteristic'
declare @ccTemplateTable sysname
    set @ccTemplateTable = 'cc_checksum_land_detail_characteristic_template' -- this is also hard coded in one spot  
declare @new_table_name sysname
    set @new_table_name = '#ccCheckSum_land_detail_characteristic_new' -- this is also hard coded in one spot
declare @old_table_name sysname
    set @old_table_name = 'ccCheckSum_land_detail_characteristic_old'
...
select * into #ccCheckSum_land_detail_characteristic_new 
  from dbo.cc_checksum_land_detail_characteristic_template
