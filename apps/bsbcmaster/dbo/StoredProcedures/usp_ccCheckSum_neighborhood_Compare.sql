...
select @data_type_id = data_type_id from dbo.ccChecksum_TrackedData where data_desc = 'neighborhood'

exec dbo.usp_ccCheckSum_neighborhood_Compare 2,2011,-7,@data_type_id,1

 
*/
 
CREATE PROCEDURE dbo.usp_ccCheckSum_neighborhood_Compare
...
declare @TableName sysname 
    set @TableName = 'neighborhood'
declare @ccTemplateTable sysname
    set @ccTemplateTable = 'cc_checksum_neighborhood_template' -- this is also hard coded in one spot  
declare @new_table_name sysname
    set @new_table_name = '#ccCheckSum_neighborhood_new' -- this is also hard coded in one spot
declare @old_table_name sysname
    set @old_table_name = 'ccCheckSum_neighborhood_old'
declare @join_table_for_prop_id sysname
    set @join_table_for_prop_id = 'property_val' 
declare @join_syntax varchar(300)
    set @join_syntax = ' ON ' + @TableName + '.hood_yr = '
                     + @join_table_for_prop_id + '.prop_val_yr '   
                     + ' AND ' + @TableName + '.hood_cd = ' 
                     + @join_table_for_prop_id + '.hood_cd '
...
-- both tables have a field called cycle, so prefix name with tracked table name
set @TrackedColumns = replace(@TrackedColumns,'[cycle]', @TableName + '.[cycle]') 
...
select * into #ccCheckSum_neighborhood_new 
  from dbo.cc_checksum_neighborhood_template
...
set @sql = ' insert into ' + @new_table_name + @CRLF
         ...
         + ' and ' +  @TableName + '.hood_yr =  ' + convert(varchar(4),@yr) + @CRLF + @CRLF
