...
select @data_type_id = data_type_id from dbo.ccChecksum_TrackedData where data_desc = 'ccProperty'
print @data_type_id
exec dbo.usp_ccCheckSum_ccProperty_Compare 2,2012,-2,@data_type_id,1
*/
 
CREATE PROCEDURE dbo.usp_ccCheckSum_ccProperty_Compare
...
declare @TableName sysname 
    set @TableName = 'ccProperty'
declare @ccTemplateTable sysname
    set @ccTemplateTable = 'cc_checksum_ccProperty_template' -- this is also hard coded in one spot  
declare @new_table_name sysname
    set @new_table_name = '#ccCheckSum_ccProperty_new' -- this is also hard coded in one spot
declare @old_table_name sysname
    set @old_table_name = 'ccCheckSum_ccProperty_old'
...
select * into #ccCheckSum_ccProperty_new 
  from dbo.cc_checksum_ccProperty_template
...
if @old_table_exists = 1 and @run_type = 2
   begin
     ...
     set @sql = 'insert into  dbo.ccCheckSum_Compare_RunInfo_Details '
              ...
              + ', 1 as valid_for_cloud,case isnull(old.prop_id,-1)  when - 1 then 1 else 0 end as new_for_cloud,0 as inserted_to_queue_table '  
