CREATE PROCEDURE CreateFY_NY_SL_income_sched_imprv_econ_matrix_assoc
...
INSERT INTO 
		income_sched_imprv_econ_matrix_assoc
(
		[year],
		economic_area,
		imprv_det_type_cd,
...
SELECT 
 ...
 FROM 
    income_sched_imprv_econ_matrix_assoc as prf LEFT JOIN 
     (select @lInputFromYear as [year],economic_area,imprv_det_type_cd,imprv_det_meth_cd,matrix_id,matrix_order
        from income_sched_imprv_econ_matrix_assoc with (nolock) 
       where [year] = @lCopyToYear) as fy_prf
   on prf.[year] = fy_prf.[year]
 and prf.economic_area = fy_prf.economic_area
