...
select @data_type_id = data_type_id from dbo.ccChecksum_TrackedData where data_desc = 'imprv_detail_cms_section'
print @data_type_id
exec dbo.usp_ccCheckSum_imprv_detail_cms_section_Compare 2,2012,-9,@data_type_id,1
*/

 
CREATE PROCEDURE dbo.usp_ccCheckSum_imprv_detail_cms_section_Compare
...
declare @TableName sysname 
    set @TableName = 'imprv_detail_cms_section'
declare @ccTemplateTable sysname
    set @ccTemplateTable = 'cc_checksum_imprv_detail_cms_section_template' -- this is also hard coded in one spot  
declare @new_table_name sysname
    set @new_table_name = '#ccCheckSum_imprv_detail_cms_section_new' -- this is also hard coded in one spot
declare @old_table_name sysname
    set @old_table_name = 'ccCheckSum_imprv_detail_cms_section_old'
...
select * into #ccCheckSum_imprv_detail_cms_section_new 
  from dbo.cc_checksum_imprv_detail_cms_section_template
