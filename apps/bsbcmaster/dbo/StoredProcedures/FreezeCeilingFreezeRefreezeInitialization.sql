...
create procedure FreezeCeilingFreezeRefreezeInitialization
	@run_id int
as
...
exec FreezeCeilingFreezeRefreezeDeletion @run_id
...
-- Retrieve HS exemption for the year associated with the freeze ceiling run
update
	freeze_ceiling_freeze_refreeze_run_detail
set
	hs_exemption = ltrim(rtrim(pe.exmpt_type_cd))
from
	freeze_ceiling_run as fcr with (nolock)
inner join
	freeze_ceiling_freeze_refreeze_run_detail as detail with (nolock)
...
and	ltrim(rtrim(pe.exmpt_type_cd)) = 'HS'
where
	fcr.run_id = @run_id
...
set
	...
	actual_new_val_hs = poev.actual_new_val_hs,
	actual_new_val_nhs = poev.actual_new_val_nhs,
	actual_new_val_taxable = poev.actual_new_val_taxable,
...
-- Retrieve tax rate info for the year associated with the freeze ceiling run
update
	freeze_ceiling_freeze_refreeze_run_detail
set
	m_n_o_tax_pct = tr.m_n_o_tax_pct,
	i_n_s_tax_pct = tr.i_n_s_tax_pct,
...
-- Retrieve HS exemption for the previous year
update
	freeze_ceiling_freeze_refreeze_run_detail
set
	prev_yr_hs_exemption = ltrim(rtrim(pe.exmpt_type_cd))
from
	freeze_ceiling_run as fcr with (nolock)
inner join
	freeze_ceiling_freeze_refreeze_run_detail as detail with (nolock)
...
set
	...
	prev_yr_actual_new_val_hs = poev.actual_new_val_hs,
	prev_yr_actual_new_val_nhs = poev.actual_new_val_nhs,
	prev_yr_actual_new_val_taxable = poev.actual_new_val_taxable,
...
-- Set the flag to restore missing freeze
update
	freeze_ceiling_freeze_refreeze_run_detail
set
	missing_freeze = 1
...
and	detail.qualify_yr < (detail.tax_yr - 1)
where
	fcr.run_id = @run_id



-- Set the flag to set a freeze when the qualify year is the previous year
update
	freeze_ceiling_freeze_refreeze_run_detail
set
	qualify_year_freeze = 1
from
	freeze_ceiling_run as fcr with (nolock)
inner join
	freeze_ceiling_freeze_refreeze_run_detail as detail with (nolock)
...
-- Set the flag for a refreeze when the qualify year is the previous year and an HS exemption is added in the selected year
update
	freeze_ceiling_freeze_refreeze_run_detail
set
	hs_exemption_refreeze = 1
from
	freeze_ceiling_run as fcr with (nolock)
inner join
	freeze_ceiling_freeze_refreeze_run_detail as detail with (nolock)
on
	detail.run_id = fcr.run_id
and	detail.qualify_yr = (detail.tax_yr - 1)
and	detail.hs_exemption = 'HS'
and	isnull(detail.prev_yr_hs_exemption, '') <> 'HS'
where
	fcr.run_id = @run_id



-- Set the flag for a refreeze when there is new value
update
	freeze_ceiling_freeze_refreeze_run_detail
set
	new_value_refreeze = 1
from
	freeze_ceiling_run as fcr with (nolock)
inner join
	freeze_ceiling_freeze_refreeze_run_detail as detail with (nolock)
...
and	detail.actual_new_val_hs > 0
where
	fcr.run_id = @run_id



-- Set the flag for a freeze when there is a transfer
update
	freeze_ceiling_freeze_refreeze_run_detail
set
	transfer_freeze = 1
...
on
	detail.run_id = fcr.run_id
and	detail.qualify_yr = detail.tax_yr
and	detail.transfer_dt is not null
and	datepart(yyyy, detail.transfer_dt) = detail.tax_yr
where
	fcr.run_id = @run_id
