...
CREATE PROCEDURE SalesCompEquityTaxpayerEvidenceReports
...
AS
    ...
    select '
	Set @strSQL = @strSQL + CAST(@@spid AS varchar) + ', '
	Set @strSQL = @strSQL + ''''+ @type  + ''', '
	...
	isnull(iv.CAPR,0),
	p.dba_name, '
	if( @type = 'RS' ) begin Set @strSQL = @strSQL + 'isnull(sa.sl_price,0), case when isnull(pp.living_area,0) >0 then isnull(sa.sl_price,0) / isnull(pp.living_area,0) else 0 end ' end
	else begin Set @strSQL = @strSQL + '0, 0, ' end
	...
	if( @type = 'RS' ) begin Set @strSQL = @strSQL + 'sa.sl_dt, ' end
	else begin Set @strSQL = @strSQL + ''''', ' end
	...
	where 	pv.prop_id in(
		select prop_id from ##temp_propid_spid
		where spid = ' 
		Set @strSQL = @strSQL + CAST( @@spid AS varchar )
		Set @strSQL = @strSQL + 'and type= '
		Set @strSQL = @strSQL + '''' + @type + ''''
		Set @strSQL = @strSQL + ' )
		and pv.prop_val_yr= ' 
		Set @strSQL = @strSQL + CAST( @yr AS varchar )
		Set @strSQL = @strSQL + '
 --	31389
