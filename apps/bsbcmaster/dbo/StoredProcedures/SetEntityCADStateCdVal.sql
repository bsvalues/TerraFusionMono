...
create procedure SetEntityCADStateCdVal
...
begin
	...
	begin
		declare property_val_cad_state_cd_cursor CURSOR FAST_FORWARD
		...
		open property_val_cad_state_cd_cursor
		fetch next from property_val_cad_state_cd_cursor into
			   @pvs_assessed_val,      
	   		   @pvs_land_hstd_val,    
		...
		begin
			...
			insert into property_owner_entity_cad_state_cd with(tablock)
			(
			prop_id,     
			year,        
			sup_num,     
		...
		close property_val_cad_state_cd_cursor
		deallocate property_val_cad_state_cd_cursor 
