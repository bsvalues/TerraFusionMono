...
/*
exec dbo.usp_ccCheckSum_address_Compare 2,2013,1,1,1
*/
 
CREATE PROCEDURE dbo.usp_ccCheckSum_address_Compare
...
declare @ccTemplateTable sysname
    set @ccTemplateTable = 'cc_checksum_address_template' -- this is also hard coded in one spot  
declare @new_table_name sysname
    set @new_table_name = '#ccCheckSum_address_new' -- this is also hard coded in one spot
declare @old_table_name sysname
    set @old_table_name = 'ccCheckSum_address_old'
declare @join_table_for_prop_id sysname
    set @join_table_for_prop_id = 'owner' -- pk prop_id, bldg_permit_id
declare @join_syntax varchar(300)
    set @join_syntax = ' ON ' + @TableName + '.acct_id = '
                     + @join_table_for_prop_id + '.prop_id'   
...
select * into #ccCheckSum_address_new 
  from dbo.cc_checksum_address_template
...
set @sql = ' insert into ' + @new_table_name + @CRLF
         ...
         + @join_syntax + @CRLF 
         + ' where ' + @join_table_for_prop_id + '.owner_tax_yr = ' + convert(varchar(4),@yr) + @CRLF + @CRLF
