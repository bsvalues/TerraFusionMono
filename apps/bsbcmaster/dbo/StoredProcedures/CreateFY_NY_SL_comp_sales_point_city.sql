CREATE   PROCEDURE CreateFY_NY_SL_comp_sales_point_city
...
SELECT 
    ...
    ,@lCopyToYear
 FROM 
    comp_sales_point_city as cspc LEFT JOIN 
     (select @lInputFromYear as lYear,lCityDiff
        from comp_sales_point_city with (nolock) 
       where lYear = @lCopyToYear) as fy_cspc
   on cspc.lYear = fy_cspc.lYear
 and cspc.lCityDiff = fy_cspc.lCityDiff
