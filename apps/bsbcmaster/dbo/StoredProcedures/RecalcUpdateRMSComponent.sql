...
create procedure RecalcUpdateRMSComponent
...
set nocount on
	...
	set @szSQL = '
		bulk insert #recalc_bcp_imprv_detail_rms_component
		from ''' + @szBCPFile + '''
		with
		(
			maxerrors = 0,
	...
	begin
		...
		set
			...
			rms.UnitPrice = trms.UnitPrice,
			rms.AdjUnitPrice = trms.AdjUnitPrice,
			rms.ComponentValueRCN = trms.ComponentValueRCN,
			rms.ComponentValueRCNLD = trms.ComponentValueRCNLD
		from imprv_detail_rms_component as rms
		join #recalc_bcp_imprv_detail_rms_component as trms with(nolock) on
			...
			rms.pacs_component_id = trms.pacs_component_id and
