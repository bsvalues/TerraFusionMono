...
create procedure ChangeLogSelectImprvAdjType
	@szImprvAdjType varchar(24),
	@lYear int
as
	...
	from change_log_keys as imprv_type with(nolock)
	join change_log_keys as imprv_year with(nolock) on
		imprv_year.lChangeID = imprv_type.lChangeID and
		imprv_year.iColumnID = 2205 and
		imprv_year.lKeyValue = @lYear
	join change_log as cl with(nolock) on
		cl.lChangeID = imprv_type.lChangeID
	...
	where
		imprv_type.iColumnID = 2201 and
		imprv_type.szKeyValue = @szImprvAdjType
