...
select @data_type_id = data_type_id from dbo.ccChecksum_TrackedData where data_desc = 'main_image'
print @data_type_id
exec dbo.usp_ccCheckSum_main_image_Compare 2,2011,-10,@data_type_id,1

*/
 
CREATE PROCEDURE dbo.usp_ccCheckSum_main_image_Compare
...
declare @TableName sysname 
    set @TableName = 'property_val'
declare @ccTemplateTable sysname
    set @ccTemplateTable = 'cc_checksum_main_image_template' -- this is also hard coded in one spot  
declare @new_table_name sysname
    set @new_table_name = '#ccCheckSum_main_image_new' -- this is also hard coded in one spot
declare @old_table_name sysname
    set @old_table_name = 'ccCheckSum_main_image_old'
...
SET @sql = 'select ' + @PKColNames 
         + ', image_path '
         + ', cast(0 as int) as checksum_val into ' 
...
select * into #ccCheckSum_main_image_new 
  from dbo.cc_checksum_main_image_template

set @StepMsg = 'Step 5 '

-- now set up sql to get this year data
set @TrackedColumns = 'image_path'
 
set @sql = ' insert into ' + @new_table_name + @CRLF
         + ' select ' + @PKColNames + ',image_path,CHECKSUM(' + @TrackedColumns + ') as checksum_val'  + @CRLF
         + ' from ' + @TableName  + @CRLF
...
if @old_table_exists = 1 and @run_type = 2
   begin
     ...
     set @StepMsg = 'Step 8 '
     -- only need to record new value in image_path  -- as of dec. 20, 2013
     -- decision is to not send any blank image_path values to cloud via sync table
     -- full join to recognize adds and deletes
     set @sql = 'insert into  dbo.ccCheckSum_Compare_RunInfo_Details ' + @CRLF
              + ' (run_id,data_type_id,prop_id,IsNew,IsDel,PKColNames,PKValues' + @CRLF
              + ',valid_for_cloud,new_for_cloud,inserted_to_queue_table,image_path) ' + @CRLF
              ...
              + @PKColJoin  + @CRLF
              + 'where (new.prop_id is null or old.prop_id is null or new.checksum_val <> old.checksum_val)'  + @CRLF  + @CRLF
     if @debug = 1
        begin 
          print @sql
        end
