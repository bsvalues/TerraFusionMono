...
CREATE procedure PrepareMortgageCertStatement
@input_mortgage_id	int,
@input_user_id		int,
...
DECLARE MORTGAGE_PROPERTY_BILL SCROLL CURSOR
FOR select entity.entity_cd,
	   ...
	   bill.stmnt_id,
           bill.prop_id,
    	   bill.sup_tax_yr
    from   bill, entity, mortgage_assoc
    where  mortgage_assoc.prop_id 		= bill.prop_id
    and    mortgage_assoc.mortgage_co_id 	= @input_mortgage_id
...
OPEN MORTGAGE_PROPERTY_BILL
FETCH NEXT FROM  MORTGAGE_PROPERTY_BILL into @entity_cd, 
	@owner_id, 
	@entity_id,
	@bill_id,  
	@stmnt_id,  
...
/* delete any entry for the user/mortgage_co combo */
delete from mortgage_prop_tax_due
where pacs_user_id = @input_user_id
and   mortgage_co_id = @input_mortgage_id
...
begin
	...
	begin
		...
		insert into mortgage_prop_tax_due
		(
	 		pacs_user_id,
			mortgage_co_id,
		 	prop_id,
...
CLOSE MORTGAGE_PROPERTY_BILL
DEALLOCATE MORTGAGE_PROPERTY_BILL
