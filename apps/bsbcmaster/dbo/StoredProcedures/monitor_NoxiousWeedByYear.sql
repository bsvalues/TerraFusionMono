...
CREATE procedure [dbo].[monitor_NoxiousWeedByYear]  
...
that do not include Weed District 1
...
select distinct p.geo_id as parcel_number, ---(select min(prop_val_yr) from property_val with(nolock) where prop_id = p.prop_id) as eff_yr, 
	...
	a.file_as_name as owner_name, 
		...
		(ad.addr_line1 + ' ' + ad.addr_line2 + ' ' + ad.addr_line3 + ' ' + 
			...
			pv.legal_acreage, wpoe.exmpt_type_cd
...
and pv.prop_id not in 
	...
	(select prop_id from property_special_assessment with(nolock)

	where year = pv.prop_val_yr
