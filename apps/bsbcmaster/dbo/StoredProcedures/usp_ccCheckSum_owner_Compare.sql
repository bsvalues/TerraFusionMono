...
/*
exec dbo.usp_ccCheckSum_owner_Compare 2,2013,1,1
*/
 
CREATE PROCEDURE dbo.usp_ccCheckSum_owner_Compare
...
declare @ccTemplateTable sysname
    set @ccTemplateTable = 'cc_checksum_owner_template' -- this is also hard coded in one spot  
declare @new_table_name sysname
    set @new_table_name = '#ccCheckSum_owner_new' -- this is also hard coded in one spot
declare @old_table_name sysname
    set @old_table_name = 'ccCheckSum_owner_old'
...
select * into #ccCheckSum_owner_new 
  from dbo.cc_checksum_owner_template
...
set @sql = ' insert into ' + @new_table_name + @CRLF
         ...
         + ' from ' + @TableName  + @CRLF
         + ' where owner_tax_yr = ' + convert(varchar(4),@yr)
