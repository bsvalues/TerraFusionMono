...
select @data_type_id = data_type_id from dbo.ccChecksum_TrackedData where data_desc = 'prop_characteristic_assoc'
print @data_type_id
exec dbo.usp_ccCheckSum_prop_characteristic_assoc_Compare 2,2011,-11,@data_type_id,1
*/
 
CREATE PROCEDURE dbo.usp_ccCheckSum_prop_characteristic_assoc_Compare
...
declare @TableName sysname 
    set @TableName = 'prop_characteristic_assoc'
declare @ccTemplateTable sysname
    set @ccTemplateTable = 'cc_checksum_prop_characteristic_assoc_template' -- this is also hard coded in one spot  
declare @new_table_name sysname
    set @new_table_name = '#ccCheckSum_prop_characteristic_assoc_new' -- this is also hard coded in one spot
declare @old_table_name sysname
    set @old_table_name = 'ccCheckSum_prop_characteristic_assoc_old'
...
select * into #ccCheckSum_prop_characteristic_assoc_new 
  from dbo.cc_checksum_prop_characteristic_assoc_template

set @StepMsg = 'Step 5 '

-- now set up sql to get this year data
-- this table only has PK columns - so make those the checksum columns
set @sql = ' insert into ' + @new_table_name + @CRLF
         + ' select ' + @PKColNames + ',CHECKSUM(' + @PKColNames + ') as checksum_val'  + @CRLF
         + ' from ' + @TableName  + @CRLF
