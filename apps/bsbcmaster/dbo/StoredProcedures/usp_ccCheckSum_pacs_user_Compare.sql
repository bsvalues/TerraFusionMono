...
select @data_type_id = data_type_id from dbo.ccChecksum_TrackedData where data_desc = 'pacs_user'

exec dbo.usp_ccCheckSum_pacs_user_Compare 2,2011,1,@data_type_id,1

*/
 
CREATE PROCEDURE dbo.usp_ccCheckSum_pacs_user_Compare
...
declare @TableName sysname 
    set @TableName = 'pacs_user'
declare @ccTemplateTable sysname
    set @ccTemplateTable = 'cc_checksum_pacs_user_template' -- this is also hard coded in one spot  
declare @new_table_name sysname
    set @new_table_name = '#ccCheckSum_pacs_user_new' -- this is also hard coded in one spot
declare @old_table_name sysname
    set @old_table_name = 'ccCheckSum_pacs_user_old'
declare @join_table_for_user_id sysname
    set @join_table_for_user_id = 'appraiser'   
declare @join_syntax varchar(300)
    set @join_syntax = ' ON ' + @TableName + '.pacs_user_id = '
                     + @join_table_for_user_id + '.pacs_user_id'   
...
set @PKColJoin = ' ON new.pacs_user_id = old.pacs_user_id ' + @PKColJoin

-- for pacs_user table - hard coding these for now
set @TrackedColumns = 'pacs_user_name,password_hash'
...
SET @sql = 'select ' + @PKColNames + ',pacs_user_name, cast(0 as int) as checksum_val into ' 
...
select * into #ccCheckSum_pacs_user_new 
  from dbo.cc_checksum_pacs_user_template

set @StepMsg = 'Step 5 '

-- now set up sql to data tracked for pacs_user

set @sql = ' insert into ' + @new_table_name + @CRLF
         + ' select distinct ' + replace(@PKColNames,'[pacs_user_id]', @TableName + '.pacs_user_id') 
         + ',pacs_user_name,CHECKSUM(' + @TrackedColumns + ') as checksum_val'  + @CRLF
         + ' from ' + @TableName  + @CRLF
         + ' join ' + @CRLF
         + @join_table_for_user_id  + @CRLF
         + @join_syntax + @CRLF 
         + ' where ' + @TableName + '.acct_exp_end is null or ' + @TableName + '.acct_exp_end > GETDATE()' 
         + @CRLF + @CRLF
...
if @old_table_exists = 1 and @run_type = 2
   begin
     ...
     set @sql = 'insert into  dbo.ccCheckSum_Compare_RunInfo_Details '
              + ' (run_id,data_type_id,pacs_user_id,IsNew,IsDel,PKColNames,PKValues,prop_id,pacs_user_name,valid_for_cloud,new_for_cloud,inserted_to_queue_table) '
              + ' select ' + convert(varchar(35),@run_id) + ' as run_id ' + @CRLF
              + ',' + convert(varchar(10),@data_type_id) + ' as data_type_id ' + @CRLF
              + ', coalesce(new.pacs_user_id,old.pacs_user_id) as pacs_user_id ' + @CRLF
              + ', case isnull(old.pacs_user_id,-1)  when - 1 then 1 else 0 end as IsNew ' + @CRLF   
              + ', case isnull(new.pacs_user_id,-1)  when - 1 then 1 else 0 end as IsDEL ' + @CRLF
              + ', '''  + @PKDelimitedNames + ''' as PKColNames ' + @CRLF 
              + ', ' + @PKOldNewSQL + ' as PKValues ' + @CRLF
              + ',0 as prop_id,'
              + ' coalesce(new.pacs_user_name,old.pacs_user_name) as pacs_user_name, ' + @CRLF
              + ' 1 as valid_for_cloud,0 as new_for_cloud,0 as inserted_to_queue_table '  + @CRLF
              + ' from ' + @new_table_name + ' as new ' + @CRLF
              + ' full join ' + @CRLF
              + @old_table_name + ' as old ' + @CRLF
              + @PKColJoin  + @CRLF
              + 'where (new.pacs_user_id is null or old.pacs_user_id is null or new.checksum_val <> old.checksum_val)'  + @CRLF 
              + @CRLF + @CRLF
