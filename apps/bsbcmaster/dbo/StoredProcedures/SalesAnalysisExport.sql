...
CREATE PROCEDURE [dbo].[SalesAnalysisExport]
...
/* SalesAnalysisExport '504', 2002, 1, 'RS' */
set nocount on

declare @lCommaPos int
declare @bNeedOR bit
declare @strClasses varchar(200)
declare @strSQL varchar(6100)
...
set @strSQL = @strSQL + 'REPLACE(isnull(s.situs_display, ''''), CHAR(13) + CHAR(10), '' '') as situs_address, '
...
set @strSQL = @strSQL + 'ii.add_factor, ii.size_adj_pct, isnull(ii.physical_pct,i.physical_pct) as physical_pct, '
set @strSQL = @strSQL + 'case when ii.economic_pct_override = ''T'' then ii.economic_pct else i.economic_pct end as economic_pct, '
set @strSQL = @strSQL + 'ii.functional_pct, ii.dep_pct, ii.imprv_det_calc_val, ii.imprv_det_adj_factor,  '
...
begin
	...
	begin
		...
		begin
			...
			set @bNeedOR = 1
			set @input_neighborhoods = ltrim(right(@input_neighborhoods, len(@input_neighborhoods) - @lCommaPos))
			set @lCommaPos = charindex(',', @input_neighborhoods, 1)
		end

		if len(@input_neighborhoods) > 0
		begin
			set @strSQL = @strSQL + 'or pv.hood_cd = ' + @input_neighborhoods
		end
	end
	else
	begin
