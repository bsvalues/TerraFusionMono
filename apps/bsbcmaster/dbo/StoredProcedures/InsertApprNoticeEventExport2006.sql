...
CREATE PROCEDURE InsertApprNoticeEventExport2006
...
begin
    ...
    if (exists(SELECT prop_id FROM ##appr_notice_prop_id_success_list WHERE prop_id = @prop_id AND dataset_id = @input_dataset_id))
    BEGIN
    	insert into event
    	(
    		event_id,
