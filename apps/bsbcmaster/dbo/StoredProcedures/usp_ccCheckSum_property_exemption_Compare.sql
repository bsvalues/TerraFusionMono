...
/*
exec dbo.usp_ccCheckSum_property_exemption_Compare 2,2013,1,1
*/
 
CREATE PROCEDURE dbo.usp_ccCheckSum_property_exemption_Compare
...
declare @TableName sysname 
    set @TableName = 'property_exemption'
declare @ccTemplateTable sysname
    set @ccTemplateTable = 'cc_checksum_property_exemption_template' -- this is also hard coded in one spot  
declare @new_table_name sysname
    set @new_table_name = '#ccCheckSum_property_exemption_new' -- this is also hard coded in one spot
declare @old_table_name sysname
    set @old_table_name = 'ccCheckSum_property_exemption_old'
...
select * into #ccCheckSum_property_exemption_new 
  from dbo.cc_checksum_property_exemption_template
...
set @sql = ' insert into ' + @new_table_name + @CRLF
         ...
         + ' from ' + @TableName  + @CRLF
         + ' where exmpt_tax_yr = ' + convert(varchar(4),@yr)
