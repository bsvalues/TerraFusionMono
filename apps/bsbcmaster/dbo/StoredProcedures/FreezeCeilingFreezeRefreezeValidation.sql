...
create procedure FreezeCeilingFreezeRefreezeValidation
	@run_id int
as
...
and	isnull(detail.hs_exemption_refreeze, 0) = 0
and	isnull(detail.new_value_refreeze, 0) = 0
and	isnull(detail.transfer_freeze, 0) = 0
where
	fcr.run_id = @run_id
...
or	detail.hs_exemption_refreeze = 1
or	detail.new_value_refreeze = 1
or	detail.transfer_freeze = 1
)
where
	fcr.run_id = @run_id
...
-- Have the tax rates been set for the current year?
update
	freeze_ceiling_freeze_refreeze_run_detail
set
	action_indicator = 1,
	action_message = 'Tax Rates are not set for the current Tax Year.',
	report_sort_order = 210,
	calculated_freeze_ceiling = null,
	calculated_freeze_yr = null
from
	freeze_ceiling_run as fcr with (nolock)
...
and	detail.action_indicator < 0
and	detail.report_sort_order < 0
and	isnull(m_n_o_tax_pct, 0.0) = 0.0
and	isnull(i_n_s_tax_pct, 0.0) = 0.0
and
(
	detail.hs_exemption_refreeze = 1
or	detail.new_value_refreeze = 1
or	detail.transfer_freeze = 1
)
where
...
-- Does the qualify year for the exemption match the tax year?
update
	freeze_ceiling_freeze_refreeze_run_detail
set
	action_indicator = 1,
	action_message = 'Qualify Year for the Exemption is not equal to the Tax Year.',
...
or
	(
		detail.new_value_refreeze = 1
	and	detail.qualify_year_freeze = 0
	and	detail.hs_exemption_refreeze = 0
	)
)
...
-- Is transfer percentage present.
update
	freeze_ceiling_freeze_refreeze_run_detail
set
	action_indicator = 1,
	action_message = 'Transfer Percentage is not available.',
	report_sort_order = 740,
	calculated_freeze_ceiling = null,
	calculated_freeze_yr = null
from
	freeze_ceiling_run as fcr with (nolock)
...
and	detail.action_indicator < 0
and	detail.report_sort_order < 0
and	isnull(detail.transfer_pct, -1.0) < 0.0
and	detail.transfer_freeze = 1
where
	fcr.run_id = @run_id
