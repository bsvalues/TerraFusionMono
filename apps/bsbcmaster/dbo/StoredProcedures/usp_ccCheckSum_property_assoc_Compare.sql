...
select @data_type_id = data_type_id from dbo.ccChecksum_TrackedData where data_desc = 'property_assoc'
print @data_type_id
exec dbo.usp_ccCheckSum_property_assoc_Compare 2,2011,-4,@data_type_id,1
*/
 
CREATE PROCEDURE dbo.usp_ccCheckSum_property_assoc_Compare
...
declare @TableName sysname 
    set @TableName = 'property_assoc'
declare @ccTemplateTable sysname
    set @ccTemplateTable = 'cc_checksum_property_assoc_template' -- this is also hard coded in one spot  
declare @new_table_name sysname
    set @new_table_name = '#ccCheckSum_property_assoc_new' -- this is also hard coded in one spot
declare @old_table_name sysname
    set @old_table_name = 'ccCheckSum_property_assoc_old'
...
select * into #ccCheckSum_property_assoc_new 
  from dbo.cc_checksum_property_assoc_template

set @StepMsg = 'Step 5 '

-- now set up sql to get this year data 
-- there are no specific columns to track on this table outside of PK, so set up the ones we want manually
set @TrackedColumns = @PKColNames 
...
if @old_table_exists = 1 and @run_type = 2
   begin
     ...
     set @sql = 'insert into  dbo.ccCheckSum_Compare_RunInfo_Details '
              + ' (run_id,data_type_id,prop_id,IsNew,IsDel,PKColNames,PKValues,valid_for_cloud,new_for_cloud,inserted_to_queue_table,child_prop_id) ' + @CRLF
              + ' select ' + convert(varchar(35),@run_id) + ' as run_id ' + @CRLF
              + ',' + convert(varchar(10),@data_type_id) + ' as data_type_id ' + @CRLF
              + ', coalesce(new.parent_prop_id,old.parent_prop_id) as prop_id ' + @CRLF
              + ', case isnull(old.parent_prop_id,-1)  when - 1 then 1 else 0 end as IsNew ' + @CRLF   
              + ', case isnull(new.parent_prop_id,-1)  when - 1 then 1 else 0 end as IsDEL ' + @CRLF 
              + ', ''' + @PKDelimitedNames + ''' as PKColNames ' + @CRLF 
              + ', ' + @PKOldNewSQL + ' as PKValues ' + @CRLF
              + ', 0 as valid_for_cloud,0 as new_for_cloud,0 as inserted_to_queue_table '   + @CRLF
              + ', coalesce(new.child_prop_id,old.child_prop_id) as child_prop_id ' + @CRLF
              + ' from ' + @new_table_name + ' as new ' + @CRLF
              + ' full join ' + @CRLF
              + @old_table_name + ' as old ' + @CRLF
              + @PKColJoin  + @CRLF
              + 'where (new.parent_prop_id is null or old.parent_prop_id is null or new.checksum_val <> old.checksum_val)'  + @CRLF 
     if @debug = 1
        begin 
          print @sql
        end
