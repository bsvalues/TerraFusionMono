...
CREATE procedure ptd_mt_prescan_codes
...
begin
	...
	begin
		...
		exec ptd_mt_insert_error 'PTD', 0, @ec_taxing_unit_num, @message, @dataset
	end
	else
	begin
		...
		begin
			...
			exec ptd_mt_insert_error 'PTD', 0, 'null', @message, @dataset
		end
		else
		begin
--per roy, this needs to be taken out.HS 30117
...
FOR select distinct imprv_state_cd, ptd_state_cd from imprv,state_code, ##ptd_supp_assoc
	...
	and ##ptd_supp_assoc.dataset_id = @dataset
...
begin
	...
	begin
		...
		begin
			...
			exec ptd_mt_insert_error 'PTD', 0, @state_land_type_desc, @message, @dataset
		end
	end
...
begin
	...
	exec ptd_mt_insert_error 'PTD', @prop_id, 'null', @message, @dataset
...
begin
	...
	exec ptd_mt_insert_error 'PTD', @prop_id, @prop_type_cd, @message, @dataset
