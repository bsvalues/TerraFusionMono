...
/*
exec dbo.usp_ccCheckSum_Imprv_Compare 2,2013,1,1
*/
 
CREATE PROCEDURE dbo.usp_ccCheckSum_imprv_Compare
...
declare @ccTemplateTable sysname
    set @ccTemplateTable = 'cc_checksum_imprv_template' -- this is also hard coded in one spot  
declare @new_table_name sysname
    set @new_table_name = '#ccCheckSum_imprv_new' -- this is also hard coded in one spot
declare @old_table_name sysname
    set @old_table_name = 'ccCheckSum_Imprv_old'
...
select * into #ccCheckSum_imprv_new 
  from dbo.cc_checksum_imprv_template
