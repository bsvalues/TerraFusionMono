...
--monitor command to run this monitor -- {call IslandBatchReport('6/1/2011','6/2/2011')}


CREATE procedure [dbo].[IslandBatchReport]
...
select ct.batch_id, ba.balance_dt, ba.close_dt, ba.create_dt,
      ...
      sum(ct.base_amount_pd + ct.penalty_amount_pd + ct.interest_amount_pd
			...
			+ other_amount_pd) total_paid from 

coll_transaction ct with(nolock) join batch ba with(nolock)
