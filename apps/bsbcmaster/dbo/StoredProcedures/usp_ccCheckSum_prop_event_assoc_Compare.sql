...
select @data_type_id = data_type_id from dbo.ccChecksum_TrackedData where data_desc = 'prop_event_assoc'

exec dbo.usp_ccCheckSum_prop_event_assoc_Compare 2,2011,3016,@data_type_id,1
*/
 
CREATE PROCEDURE dbo.usp_ccCheckSum_prop_event_assoc_Compare
...
AS 
/* NOTE:
   this is a bit different than other processes - since most data being tracked is in the event table
   but the table we need to capture key columns on to report Delete information is the prop_event_assoc
   so a bit more hard coding in this procedure than in others.
*/
...
declare @TableName sysname 
    set @TableName = 'event'  -- event fields are being tracked for change, prop_event_assoc has to be captured for DELETE Info
declare @ccTemplateTable sysname
    set @ccTemplateTable = 'cc_checksum_prop_event_assoc_template' -- this is also hard coded in one spot  
declare @new_table_name sysname
    set @new_table_name = '#ccCheckSum_prop_event_assoc_new' -- this is also hard coded in one spot
declare @old_table_name sysname
    set @old_table_name = 'ccCheckSum_prop_event_assoc_old'
declare @join_table_for_prop_id sysname
    set @join_table_for_prop_id = 'prop_event_assoc' 
declare @join_syntax varchar(300)
    set @join_syntax = ' ON ' + @TableName + '.event_id = '
                     + @join_table_for_prop_id + '.event_id '   
...
declare @PKColNames_WithPrefix varchar(2000)
...
set @PKColNames_WithPrefix = @TableName + '.' + replace(@PKColNames,', ',', ' + @TableName + '.')
-- WANT TO OVERRIDE TRACKED COLUMNS INFO FROM TABLE AND JUST USE THE FOLLOWING FIELDS FOR TRACKING
set @TrackedColumns = 'event_type, event_date, pacs_user, event_desc'
-- overide @PKDelimitedNames 
set @PKDelimitedNames = 'prop_id|event_id'
--print @PKColNames

--print @PKColNames_WithPrefix
--print ''
...
SET @sql = 'select cast(0 as int) as prop_id,event_id, cast(0 as int) as checksum_val into ' 
         +  @ccTemplateTable + ' from ' + @TableName -- @tableName is event here
         + ' where 1=2 ' 
...
select * into #ccCheckSum_prop_event_assoc_new 
  from dbo.cc_checksum_prop_event_assoc_template

set @StepMsg = 'Step 5 '

-- now set up sql to get this year data 

/* !! The select fields are different here than other similar procedures - gets prop_id and event_id values from prop_event_assoc table
      yet the tracked columns are from the event table
*/
print ''
set @sql = ' insert into ' + @new_table_name + @CRLF
         + ' select DISTINCT ' + @join_table_for_prop_id + '.prop_id,' + @CRLF
         +  @join_table_for_prop_id + '.event_id '  + @CRLF
         + ',CHECKSUM(' + @TrackedColumns + ') as checksum_val'  + @CRLF
         + ' from ' + @TableName  + @CRLF  -- table name is event to get tracked fields
         + ' join ' + @CRLF
         + @join_table_for_prop_id  + @CRLF
         + @join_syntax + @CRLF 
         
-- requirements says no year filter         
         --+ ' where ' + @join_table_for_prop_id + '.prop_val_yr = ' + convert(varchar(4),@yr) + @CRLF
         --+ ' and ' +  @TableName + '.event_year =  ' + convert(varchar(4),@yr) + @CRLF + @CRLF
...
set @PKColNames = 'prop_id,event_id' 
