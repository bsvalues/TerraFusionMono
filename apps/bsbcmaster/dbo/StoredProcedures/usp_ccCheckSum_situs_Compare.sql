...
/*
exec dbo.usp_ccCheckSum_situs_Compare 2,2013,1,1
*/
 
CREATE PROCEDURE dbo.usp_ccCheckSum_situs_Compare
...
declare @ccTemplateTable sysname
    set @ccTemplateTable = 'cc_checksum_situs_template' -- this is also hard coded in one spot  
declare @new_table_name sysname
    set @new_table_name = '#ccCheckSum_situs_new' -- this is also hard coded in one spot
declare @old_table_name sysname
    set @old_table_name = 'ccCheckSum_situs_old'
...
select * into #ccCheckSum_situs_new 
  from dbo.cc_checksum_situs_template
