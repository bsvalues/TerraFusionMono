...
select @data_type_id = data_type_id from dbo.ccChecksum_TrackedData where data_desc = 'imprv_adj_type'

exec dbo.usp_ccCheckSum_imprv_adj_type_Compare 2,2011,1011,@data_type_id,1
*/
 
CREATE PROCEDURE dbo.usp_ccCheckSum_imprv_adj_type_Compare
...
declare @TableName sysname 
    set @TableName = 'imprv_adj_type'
declare @ccTemplateTable sysname
    set @ccTemplateTable = 'cc_checksum_imprv_adj_type_template' -- this is also hard coded in one spot  
declare @new_table_name sysname
    set @new_table_name = '#ccCheckSum_imprv_adj_type_new' -- this is also hard coded in one spot
declare @old_table_name sysname
    set @old_table_name = 'ccCheckSum_imprv_adj_type_old'
declare @join_table_for_prop_id sysname
    set @join_table_for_prop_id = 'imprv_adj' 
declare @join_syntax varchar(300)
    set @join_syntax = ' ON ' + @TableName + '.imprv_adj_type_year = '
                     + @join_table_for_prop_id + '.prop_val_yr '   
                     + ' AND ' + @TableName + '.imprv_adj_type_cd = ' 
                     + @join_table_for_prop_id + '.imprv_adj_type_cd '
...
select * into #ccCheckSum_imprv_adj_type_new 
  from dbo.cc_checksum_imprv_adj_type_template
...
set @sql = ' insert into ' + @new_table_name + @CRLF
         ...
         + ' and ' +  @TableName + '.imprv_adj_type_year =  ' + convert(varchar(4),@yr) + @CRLF + @CRLF
