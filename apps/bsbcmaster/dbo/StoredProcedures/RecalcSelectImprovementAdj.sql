
create procedure RecalcSelectImprovementAdj
	@lPacsUserID bigint,
	@lPropID int,
	@lYear numeric(4,0),
	@lSupNum int,
	@lSaleID int = 0
as

	if ( @lPacsUserID != 0 )
	begin
			select
				ia.prop_id,
				convert(smallint, ia.prop_val_yr),
				convert(smallint, ia.sup_num),
				ia.imprv_id,
				ia.imprv_adj_seq,
				ia.imprv_adj_amt,
				ia.imprv_adj_pc,
				upper(rtrim(ia.imprv_adj_type_cd)),
				convert(smallint, ia.year_added),
				ia.imprv_adj_method
			from #recalc_prop_list as rpl with(nolock)
			join imprv_adj as ia with(nolock) on
				rpl.prop_id = ia.prop_id and
				rpl.sup_yr = ia.prop_val_yr and
				rpl.sup_num = ia.sup_num and
				ia.sale_id = @lSaleID
			order by
				ia.prop_id asc,
				ia.prop_val_yr asc,
				ia.sup_num asc,
				ia.imprv_id asc,
				ia.imprv_adj_seq asc
	end
	else
	begin
		if ( @lPropID = 0 )
		begin
			select
				ia.prop_id,
				convert(smallint, ia.prop_val_yr),
				convert(smallint, ia.sup_num),
				ia.imprv_id,
				ia.imprv_adj_seq,
				ia.imprv_adj_amt,
				ia.imprv_adj_pc,
				upper(rtrim(ia.imprv_adj_type_cd)),
				convert(smallint, ia.year_added),
				ia.imprv_adj_method
			from imprv_adj as ia with(nolock)
			where
				ia.prop_val_yr = @lYear and
				ia.sup_num = @lSupNum and
				ia.sale_id = @lSaleID
			order by
				ia.prop_id asc,
				ia.prop_val_yr asc,
				ia.sup_num asc,
				ia.imprv_id asc,
				ia.imprv_adj_seq asc
		end
		else
		begin
			select
				ia.prop_id,
				convert(smallint, ia.prop_val_yr),
				convert(smallint, ia.sup_num),
				ia.imprv_id,
				ia.imprv_adj_seq,
				ia.imprv_adj_amt,
				ia.imprv_adj_pc,
				upper(rtrim(ia.imprv_adj_type_cd)),
				convert(smallint, ia.year_added),
				ia.imprv_adj_method
			from imprv_adj as ia with(nolock)
			where
				ia.prop_id = @lPropID and
				ia.prop_val_yr = @lYear and
				ia.sup_num = @lSupNum and
				ia.sale_id = @lSaleID
			order by
				ia.prop_id asc,
				ia.prop_val_yr asc,
				ia.sup_num asc,
				ia.imprv_id asc,
				ia.imprv_adj_seq asc
		end
	end

	return( @@rowcount )

GO

