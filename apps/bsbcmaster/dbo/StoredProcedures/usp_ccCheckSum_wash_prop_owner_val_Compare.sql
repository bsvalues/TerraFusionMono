...
select @data_type_id = data_type_id from dbo.ccChecksum_TrackedData where data_desc = 'wash_prop_owner_val'
print @data_type_id
exec dbo.usp_ccCheckSum_wash_prop_owner_val_Compare 2,2011,-10,@data_type_id,1
*/

 
CREATE PROCEDURE dbo.usp_ccCheckSum_wash_prop_owner_val_Compare
...
declare @TableName sysname 
    set @TableName = 'wash_prop_owner_val'
declare @ccTemplateTable sysname
    set @ccTemplateTable = 'cc_checksum_wash_prop_owner_val_template' -- this is also hard coded in one spot  
declare @new_table_name sysname
    set @new_table_name = '#ccCheckSum_wash_prop_owner_val_new' -- this is also hard coded in one spot
declare @old_table_name sysname
    set @old_table_name = 'ccCheckSum_wash_prop_owner_val_old'
...
select * into #ccCheckSum_wash_prop_owner_val_new 
  from dbo.cc_checksum_wash_prop_owner_val_template
...
set @sql = ' insert into ' + @new_table_name + @CRLF
         ...
         + ' from ' + @TableName  + @CRLF
         + ' where [year] = ' + convert(varchar(4),@yr)
