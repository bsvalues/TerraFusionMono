CREATE TABLE [dbo].[property_owner_entity_state_cd] (
    ...
    CONSTRAINT [CPK_property_owner_entity_state_cd] PRIMARY KEY CLUSTERED ([year] ASC, [sup_num] ASC, [entity_id] ASC, [prop_id] ASC, [owner_id] ASC, [state_cd] ASC) WITH (FILLFACTOR = 100)
);
...
CREATE NONCLUSTERED INDEX [idx_prop_id]
    ON [dbo].[property_owner_entity_state_cd]([prop_id] ASC) WITH (FILLFACTOR = 90);


GO

CREATE NONCLUSTERED INDEX [idx_state_cd]
    ON [dbo].[property_owner_entity_state_cd]([state_cd] ASC) WITH (FILLFACTOR = 60);
