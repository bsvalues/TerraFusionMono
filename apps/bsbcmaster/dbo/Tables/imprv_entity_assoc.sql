CREATE TABLE [dbo].[imprv_entity_assoc] (
    ...
    CONSTRAINT [CPK_imprv_entity_assoc] PRIMARY KEY CLUSTERED ([prop_val_yr] ASC, [sup_num] ASC, [sale_id] ASC, [prop_id] ASC, [imprv_id] ASC, [entity_id] ASC) WITH (FILLFACTOR = 90),
    CONSTRAINT [CFK_imprv_entity_assoc_prop_val_yr_sup_num_sale_id_prop_id_imprv_id] FOREIGN KEY ([prop_val_yr], [sup_num], [sale_id], [prop_id], [imprv_id]) REFERENCES [dbo].[imprv] ([prop_val_yr], [sup_num], [sale_id], [prop_id], [imprv_id])
);
...
CREATE NONCLUSTERED INDEX [idx_prop_id]
    ON [dbo].[imprv_entity_assoc]([prop_id] ASC) WITH (FILLFACTOR = 90);
...
create trigger tr_imprv_entity_assoc_delete_ChangeLog
on imprv_entity_assoc
for delete
not for replication
as
...
if not exists (
     ...
     where
          chg_log_tables = 'imprv_entity_assoc' and
          chg_log_audit = 1
)
begin
     return
...
for
     select prop_id, sup_num, prop_val_yr, imprv_id, sale_id, entity_id from deleted
for read only
 
open curRows
fetch next from curRows into @prop_id, @sup_num, @prop_val_yr, @imprv_id, @sale_id, @entity_id
 
while ( @@fetch_status = 0 )
begin
     select @tvar_szRefID = entity.entity_cd + ' - ' + convert(varchar(12), @imprv_id)
     from entity with(nolock)
     where entity_id = @entity_id
 
     insert change_log with(rowlock) ( lPacsUserID, szSQLAccount, szMachineName, dtChange, szChangeType, iTableID, iColumnID, szOldValue, szNewValue, szRefID )
     values ( @tvar_lPacsUserID, system_user, host_name(), @tvar_dtNow, 'D', 831, 0, 'DELETED', 'DELETED', @tvar_szRefID )
...
create trigger tr_imprv_entity_assoc_insert_ChangeLog
on imprv_entity_assoc
for insert
not for replication
as
...
declare curRows cursor
for
     select prop_id, sup_num, prop_val_yr, imprv_id, sale_id, entity_id, entity_pct from inserted
for read only
 
open curRows
fetch next from curRows into @prop_id, @sup_num, @prop_val_yr, @imprv_id, @sale_id, @entity_id, @entity_pct
...
begin
     ...
     begin
          insert change_log with(rowlock) ( lPacsUserID, szSQLAccount, szMachineName, dtChange, szChangeType, iTableID, iColumnID, szOldValue, szNewValue, szRefID )
          values ( @tvar_lPacsUserID, system_user, host_name(), @tvar_dtNow, 'I', 831, 4026, null, convert(varchar(255), @prop_id), @tvar_szRefID )
          set @tvar_lChangeID = @@identity
     ...
     begin
          insert change_log with(rowlock) ( lPacsUserID, szSQLAccount, szMachineName, dtChange, szChangeType, iTableID, iColumnID, szOldValue, szNewValue, szRefID )
          values ( @tvar_lPacsUserID, system_user, host_name(), @tvar_dtNow, 'I', 831, 5002, null, convert(varchar(255), @sup_num), @tvar_szRefID )
          set @tvar_lChangeID = @@identity
     ...
     begin
          insert change_log with(rowlock) ( lPacsUserID, szSQLAccount, szMachineName, dtChange, szChangeType, iTableID, iColumnID, szOldValue, szNewValue, szRefID )
          values ( @tvar_lPacsUserID, system_user, host_name(), @tvar_dtNow, 'I', 831, 4083, null, convert(varchar(255), @prop_val_yr), @tvar_szRefID )
          set @tvar_lChangeID = @@identity
     ...
     begin
          insert change_log with(rowlock) ( lPacsUserID, szSQLAccount, szMachineName, dtChange, szChangeType, iTableID, iColumnID, szOldValue, szNewValue, szRefID )
          values ( @tvar_lPacsUserID, system_user, host_name(), @tvar_dtNow, 'I', 831, 2275, null, convert(varchar(255), @imprv_id), @tvar_szRefID )
          set @tvar_lChangeID = @@identity
     ...
     begin
          insert change_log with(rowlock) ( lPacsUserID, szSQLAccount, szMachineName, dtChange, szChangeType, iTableID, iColumnID, szOldValue, szNewValue, szRefID )
          values ( @tvar_lPacsUserID, system_user, host_name(), @tvar_dtNow, 'I', 831, 4485, null, convert(varchar(255), @sale_id), @tvar_szRefID )
          set @tvar_lChangeID = @@identity
     ...
     begin
          insert change_log with(rowlock) ( lPacsUserID, szSQLAccount, szMachineName, dtChange, szChangeType, iTableID, iColumnID, szOldValue, szNewValue, szRefID )
          values ( @tvar_lPacsUserID, system_user, host_name(), @tvar_dtNow, 'I', 831, 1757, null, convert(varchar(255), @entity_id), @tvar_szRefID )
          set @tvar_lChangeID = @@identity
     ...
     begin
          insert change_log with(rowlock) ( lPacsUserID, szSQLAccount, szMachineName, dtChange, szChangeType, iTableID, iColumnID, szOldValue, szNewValue, szRefID )
          values ( @tvar_lPacsUserID, system_user, host_name(), @tvar_dtNow, 'I', 831, 1760, null, convert(varchar(255), @entity_pct), @tvar_szRefID )
          set @tvar_lChangeID = @@identity
...
create trigger tr_imprv_entity_assoc_update_ChangeLog
on imprv_entity_assoc
for update
not for replication
as
...
declare curRows cursor
for
     select d.prop_id, d.sup_num, d.prop_val_yr, d.imprv_id, d.sale_id, d.entity_id, d.entity_pct, i.prop_id, i.sup_num, i.prop_val_yr, i.imprv_id, i.sale_id, i.entity_id, i.entity_pct
from deleted as d
...
open curRows
fetch next from curRows into @old_prop_id, @old_sup_num, @old_prop_val_yr, @old_imprv_id, @old_sale_id, @old_entity_id, @old_entity_pct, @new_prop_id, @new_sup_num, @new_prop_val_yr, @new_imprv_id, @new_sale_id, @new_entity_id, @new_entity_pct
 
while ( @@fetch_status = 0 )
begin
     select @tvar_szRefID = entity.entity_cd + ' - ' + convert(varchar(12), @new_imprv_id)
     from entity with(nolock)
     ...
     begin
          ...
          begin
               ...
               values ( @tvar_lPacsUserID, system_user, host_name(), @tvar_dtNow, 'U', 831, 4026, convert(varchar(255), @old_prop_id), convert(varchar(255), @new_prop_id) )
               set @tvar_lChangeID = @@identity
     ...
     begin
          ...
          begin
               insert change_log with(rowlock) ( lPacsUserID, szSQLAccount, szMachineName, dtChange, szChangeType, iTableID, iColumnID, szOldValue, szNewValue )
               values ( @tvar_lPacsUserID, system_user, host_name(), @tvar_dtNow, 'U', 831, 5002, convert(varchar(255), @old_sup_num), convert(varchar(255), @new_sup_num) )
               set @tvar_lChangeID = @@identity
     ...
     begin
          ...
          begin
               insert change_log with(rowlock) ( lPacsUserID, szSQLAccount, szMachineName, dtChange, szChangeType, iTableID, iColumnID, szOldValue, szNewValue )
               values ( @tvar_lPacsUserID, system_user, host_name(), @tvar_dtNow, 'U', 831, 4083, convert(varchar(255), @old_prop_val_yr), convert(varchar(255), @new_prop_val_yr) )
               set @tvar_lChangeID = @@identity
     ...
     begin
          ...
          begin
               insert change_log with(rowlock) ( lPacsUserID, szSQLAccount, szMachineName, dtChange, szChangeType, iTableID, iColumnID, szOldValue, szNewValue )
               values ( @tvar_lPacsUserID, system_user, host_name(), @tvar_dtNow, 'U', 831, 2275, convert(varchar(255), @old_imprv_id), convert(varchar(255), @new_imprv_id) )
               set @tvar_lChangeID = @@identity
     ...
     begin
          ...
          begin
               insert change_log with(rowlock) ( lPacsUserID, szSQLAccount, szMachineName, dtChange, szChangeType, iTableID, iColumnID, szOldValue, szNewValue )
               values ( @tvar_lPacsUserID, system_user, host_name(), @tvar_dtNow, 'U', 831, 4485, convert(varchar(255), @old_sale_id), convert(varchar(255), @new_sale_id) )
               set @tvar_lChangeID = @@identity
     ...
     begin
          ...
          begin
               insert change_log with(rowlock) ( lPacsUserID, szSQLAccount, szMachineName, dtChange, szChangeType, iTableID, iColumnID, szOldValue, szNewValue )
               values ( @tvar_lPacsUserID, system_user, host_name(), @tvar_dtNow, 'U', 831, 1757, convert(varchar(255), @old_entity_id), convert(varchar(255), @new_entity_id) )
               set @tvar_lChangeID = @@identity
     ...
     begin
          ...
          begin
               insert change_log with(rowlock) ( lPacsUserID, szSQLAccount, szMachineName, dtChange, szChangeType, iTableID, iColumnID, szOldValue, szNewValue )
               values ( @tvar_lPacsUserID, system_user, host_name(), @tvar_dtNow, 'U', 831, 1760, convert(varchar(255), @old_entity_pct), convert(varchar(255), @new_entity_pct) )
               set @tvar_lChangeID = @@identity
