CREATE TABLE [dbo].[freeze_ceiling_accept_supplement_group_run_detail] (
    [run_id]                            INT              NOT NULL,
    ...
    CONSTRAINT [CPK_freeze_ceiling_accept_supplement_group_run_detail] PRIMARY KEY CLUSTERED ([run_id] ASC, [prop_id] ASC, [owner_id] ASC, [entity_id] ASC, [tax_yr] ASC, [sup_num] ASC, [freeze_type] ASC) WITH (FILLFACTOR = 90),
    CONSTRAINT [CFK_freeze_ceiling_accept_supplement_group_run_detail_run_id] FOREIGN KEY ([run_id]) REFERENCES [dbo].[freeze_ceiling_run] ([run_id])
);
...
CREATE NONCLUSTERED INDEX [idx_action_indicator]
    ON [dbo].[freeze_ceiling_accept_supplement_group_run_detail]([action_indicator] ASC);


GO

CREATE NONCLUSTERED INDEX [idx_report_sort_order]
    ON [dbo].[freeze_ceiling_accept_supplement_group_run_detail]([report_sort_order] ASC);
