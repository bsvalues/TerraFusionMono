...
CREATE FUNCTION fn_GetProratePct
( 
...
AS
BEGIN
	declare @output_prorate_pct	decimal(20,19)
	...
	set @output_prorate_pct = case
		when @prorationDays > 0 then convert(decimal(22,19), @prorationDays) / convert(decimal(22,19), @daysInYear)
		else 0
	end

	RETURN (@output_prorate_pct)
END
