...
create function fn_LeaseGetTotalORInterest (@input_lease_id varchar(20), @input_lease_yr int, @input_rev_num int)
...
declare @total_or_interest_pct numeric(14,8)
set @total_or_interest_pct = 0.0
...
select
	@total_or_interest_pct = sum(isnull(lpa.interest_pct, 0.0))
from
...
and	lpa.interest_type_cd = 'OR'


return (isnull(@total_or_interest_pct, 0.0))
